...
# =============================================================================
# GREEKS HEATMAP SCORING SYSTEM (V13 FEATURE)
# =============================================================================
...
# =============================================================================
# FIX PACK (7/7): Core helpers (P&L multiplier, time gates, IV/vol scaling)
# =============================================================================

LOT_CONFIG = {
    # Fallback multipliers when lot_qty is not reliable/known
    "SENSEX": 250.0,
    "NIFTY": 75.0,
    "BANKNIFTY": 15.0,
    "FINNIFTY": 40.0,
...
def get_pnl_multiplier(symbol: str, is_option: bool, lot_qty: int) -> float:
    """Return rupees-per-point multiplier for P&L and SL/TP math.

    Priority:
    1) For options: use lot_qty if it looks sane (>1).
    2) Otherwise use LOT_CONFIG fallbacks.
    """
    s = (symbol or "").upper()
    if is_option and isinstance(lot_qty, int) and lot_qty and lot_qty > 1:
        return float(lot_qty)

    for k, v in LOT_CONFIG.items():
        if k in s:
            return float(v)

    # Safe fallback
    return float(lot_qty) if (lot_qty and lot_qty > 0) else 1.0

def is_expiry_day_from_days(days_to_expiry: int) -> bool:
    try:
        return int(days_to_expiry) <= 0
    except Exception:
        return False

def time_gate_ok(now_ist, is_expiry_day: bool) -> Tuple[bool, str]:
    """Entry time gates (Fix #3)."""
    t = now_ist.time()
    # Start after first 10 minutes
    if t < dt_time(9, 25):
        return False, "PRE_09_25"
    # Lunch dead-zone removed as per user request
    # if dt_time(12, 30) <= t < dt_time(14, 0):
    #     return False, "LUNCH_BLOCK_12_30_14_00"
    # Expiry cutoff (avoid IV crush chaos)
    if is_expiry_day and t >= dt_time(14, 15):
        return False, "EXPIRY_CUTOFF_14_15"
    # Normal day cutoff (new entries)
    if (not is_expiry_day) and t >= dt_time(15, 0):
        return False, "CUTOFF_15_00"
    return True, "OK"

def vol_target_scale(vix_now: float, iv_now: float = None, is_expiry_day: bool = False, now_ist=None) -> float:
    """Volatility-scaled TP multiplier (Fix #2). Uses IV if available else VIX."""
    ref = None
    if iv_now is not None and iv_now > 0:
        ref = iv_now
        # crude IV→regime mapping (percent IV)
        if ref >= 75:
            scale = 3.0
        elif ref >= 55:
            scale = 2.5
        ...
        ref = vix_now if vix_now is not None else 20.0
        if ref >= 35:
            scale = 3.0
        elif ref >= 25:
            scale = 2.5
    ...
    # Expiry + late day reductions
    if is_expiry_day:
        scale *= 0.70  # expiry compression
    if now_ist is not None and now_ist.time() >= dt_time(14, 30):
        scale *= 0.50  # last hour compression
    return max(1.0, round(scale, 2))

def fast_entry_mode(vix_now: float, iv_now: float = None) -> bool:
    """Fast entry mode (Fix #4): allow 2-vote entries in high volatility."""
    if iv_now is not None and iv_now >= 60:
        return True
    if vix_now is not None and vix_now >= 25:
        return True
    return False

def sl_time_adjust(sl_points: float, now_ist, is_expiry_day: bool) -> float:
    """Dynamic SL time tightening (Fix #5)."""
    t = now_ist.time()
    factor = 1.0
    if is_expiry_day:
        if t >= dt_time(13, 30):
        ...
        if t >= dt_time(15, 10):
            factor *= 0.75
    return max(0.05, round(sl_points * factor, 2))

def momentum_ok(inds: Dict[str, Any], bias: str) -> Tuple[bool, float]:
    """Momentum confirmation (Fix #7): uses MOM_Z + EFI_Z strength."""
    mom = inds.get("mom_z") if inds is not None else None
    efi = inds.get("efi_z") if inds is not None else None
    try:
        mom = float(mom)
    except Exception:
        mom = 0.0
    ...
    # strength score
    strength = (abs(mom) + abs(efi)) / 2.0

    if bias.upper().startswith("BULL"):
        ok = (mom >= 0.6 and efi >= 0.3)
    else:
        ok = (mom <= -0.6 and efi <= -0.3)

    return ok, round(strength, 3)



@dataclass
class GreekMetricsV7:
    """Container for individual Greek metrics."""
    delta: float
    gamma: float
    theta: float
    vega: float
    ...
    def validate(self) -> bool:
        """Ensure all values are numeric (not NaN/Inf)."""
        try:
            import numpy as np
            return (
            ...
            # If numpy not available for any reason, fail open (avoid hard crash)
            return True
...
class GreeksHeatmapScorerV7:
    ...
    def calculate_theta_score(self, theta: float, days_to_expiry: int = 7,
                              underlying_price: float = 21000.0) -> float:
        ...
        try:
            ...
            if days_to_expiry < 2:
                if normalized_theta < 0.10:
                    return 100.0
                elif normalized_theta < 0.25:
                    return 85.0
                elif normalized_theta < 0.60:
                    return 70.0
                elif normalized_theta < 1.2:
                    return 50.0
                else:
                    return 30.0
            else:
                if normalized_theta < 0.15:
                    return 100.0
                elif normalized_theta < 0.35:
                    return 90.0
                elif normalized_theta < 0.60:
                    return 80.0
                elif normalized_theta < 0.90:
                    return 60.0
                elif normalized_theta < 1.25:
                    return 50.0
                else:
                    return 40.0
        except Exception:
    ...
    def calculate_iv_score(self, iv: float) -> float:
        ...
        try:
            iv_val = float(iv)
            iv_pct = iv_val if iv_val > 1.0 else iv_val * 100.0
    ...
    def calculate_vega_score(self, vega: float, underlying_price: float = 21000.0) -> float:
        ...
        try:
            ...
            if normalized_vega < 0.08:
                return 100.0
            elif normalized_vega < 0.15:
                return 95.0
            elif normalized_vega < 0.25:
                return 90.0
            elif normalized_vega < 0.35:
                return 85.0
            else:
                return 60.0
        except Exception:
...
# =============================================================================
# HYBRID PROFIT TARGET SYSTEM (V13 FEATURE)
# =============================================================================
@dataclass
...
class HybridTargetResult:
    ...
    greek_target_move: float = 0.0
    greek_estimated_gain: float = 0.0
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "theta_risk": self.theta_risk.value if self.theta_risk else "UNKNOWN",
            ...
            "greek_target_move": self.greek_target_move,
            "greek_estimated_gain": self.greek_estimated_gain,
            "adjustments_summary": self.adjustments_summary
        }

class HybridProfitTargetSystem:
    """
    Complete hybrid profit target calculation system for ATM options.
    Combines: ATR + Delta + Theta + VIX + Expiry scaling
    """
    ...
    def __init__(self, logger_func=None):
        self.log = logger_func or (lambda *a, **k: None)
    ...
    def check_theta_decay_risk(self, theta: float, days_to_expiry: int,
        ...
        if theta is None or days_to_expiry is None or tp_points is None:
            return ThetaRiskAssessment(
                ...
                adjusted_tp=tp_points or 0.0
            )

        total_decay = abs(float(theta)) * int(days_to_expiry)
        decay_pct = (total_decay / tp_points * 100) if tp_points > 0 else 0.0
    ...
    def adjust_targets_for_iv_change(self, tp_points: float, sl_points: float,
        ...
        if not vix_entry or vix_entry <= 0 or not vix_now or vix_now <= 0:
            return tp_points, sl_points, "VIX_UNAVAILABLE"

        vix_change_pct = ((vix_now - vix_entry) / vix_entry) * 100.0
        ...
        return round(tp_points * adjustment, 2), round(sl_points * adjustment, 2), reason
    ...
    def apply_expiry_scaling(self, tp_points: float, days_to_expiry: int) -> Tuple[float, float, str]:
        ...
        return round(tp_points * scaling, 2), scaling, explanation
    ...
    def calculate_hybrid_targets(self, entry_price: float, atr: float,
        ...
        if entry_price is None or atr is None or atr <= 0:
            ...
            result.exit_reason = "INVALID_INPUT"
            return result
        ...
        # FIX #2: Volatility-scaled targets (use IV if present else VIX)
        now_ist = indicators.get("now_ist")
        is_expiry_day = bool(indicators.get("is_expiry_day") or is_expiry_day_from_days(indicators.get("days_to_expiry", 7)))
        vix_now = float(indicators.get("vix_now", indicators.get("vix_entry", 20.0)) or 20.0)
        iv_now = indicators.get("iv_now") or indicators.get("iv")
        try:
            iv_now = float(iv_now) if iv_now is not None else None
        except Exception:
            iv_now = None

        scale = vol_target_scale(vix_now=vix_now, iv_now=iv_now, is_expiry_day=is_expiry_day, now_ist=now_ist)
        tp_points = round(tp_points * scale, 2)
        trail_gap = round(trail_gap * max(1.0, scale * 0.85), 2)

        # FIX #5: Time-tightened SL (applied after delta adjust too; we apply base now then re-apply after delta)
        if now_ist is not None:
            sl_points = sl_time_adjust(sl_points, now_ist, is_expiry_day)

        # STEP 2: DELTA ADJUSTMENT
        delta = indicators.get("delta", 0.50)
        tp_points, sl_points, delta_level = self.adjust_targets_for_delta(tp_points, sl_points, delta)

        # Re-apply time tightening after delta adjustment (Fix #5)
        if now_ist is not None:
            sl_points = sl_time_adjust(sl_points, now_ist, is_expiry_day)

        # STEP 3: THETA DECAY CHECK
        ...
        # STEP 4: IV/VIX ADJUSTMENT
        vix_entry = indicators.get("vix_entry", 20)
        vix_now = indicators.get("vix_now", 20)
        vol_regime = self.detect_volatility_regime(float(vix_now) if vix_now is not None else 20.0)

        tp_points, sl_points, vix_reason = self.adjust_targets_for_iv_change(tp_points, sl_points, vix_entry, vix_now)

        # STEP 5: EXPIRY SCALING
        tp_points, expiry_scale, expiry_reason = self.apply_expiry_scaling(tp_points, days_exp)

        # STEP 6: GREEK-BASED TARGET ESTIMATION
        underlying_move_target = atr
        gamma = indicators.get("gamma", 0.0)
        try:
            greek_estimated_gain = (float(delta) * underlying_move_target) + (0.5 * float(gamma) * (underlying_move_target ** 2))
        except Exception:
            greek_estimated_gain = 0.0
        ...
        result.risk_reward = round(tp_points / sl_points, 2) if sl_points > 0 else 0.0
        result.trail_gap = round(trail_gap, 2)
        result.theta_risk = theta_risk.risk_level
        result.theta_decay_pct = theta_risk.decay_pct_of_tp
        result.vix_adjustment = vix_reason
        result.expiry_days = int(days_exp) if days_exp is not None else 0
        ...
        result.greek_target_move = round(underlying_move_target, 2)
        ...
        result.adjustments_summary = {
            ...
            "greek_prediction": f"Predict +₹{result.greek_estimated_gain:.2f} if underlying moves {result.greek_target_move:.1f} pts"
        }
        return result
    ...
    def log_detailed_calculation(self, entry_price: float, atr: float,
        ...
        for key, value in (result.adjustments_summary or {}).items():
            log_lines.append(f"  • {key}: {value}")

        log_lines.extend([
            "",
            "[THETA ANALYSIS]",
            f"  Risk Level: {result.theta_risk.value if result.theta_risk else 'UNKNOWN'}",
...
# =============================================================================
# COMBINED GA-RAES EXIT CONTROLLER (V13 FEATURE)
# =============================================================================
class CombinedGARaes:
    ...
    - Expiry day behavior: profit exits are partial-only until final tranche.
    """
    ...
    def greeks_health_ok(self, ltp, greeks_now):
        ...
        delta_e = float(self.pos.get("_delta_entry", 0.0) or 0.0)
        iv_e = float(self.pos.get("_iv_entry", 0.0) or 0.0)
        entry = float(self.pos.get("_entry_price", 0.0) or 0.0)

        delta_n = float(greeks_now.get("delta", 0.0) or 0.0)
        iv_n = float(greeks_now.get("iv", 0.0) or 0.0)
        ...
        # 2) IV crush
        if iv_e > 0 and iv_n >= 0 and ((iv_e - iv_n) / iv_e) >= 0.10:
            self.pos["_health_iv_fail"] = True
            return False

        # 3) Price stall (hard failure, relax on expiry)
        if entry > 0 and ltp is not None and float(ltp) <= entry * 1.01 and abs(delta_n) > 0:
            if not self.is_expiry_day():
                try:
                    self.log(f"[GA-RAES] [HEALTH] Price Stall at {ltp:.2f} (Entry: {entry:.2f}) - Hard Fail", True)
                except Exception:
                    pass
                return False
    ...
    def trend_broken(self, ltp):
        """Layer 3: Trend Break (uses pos['stop_loss'] as chandelier/ATR stop)."""
        current_sl = float(self.pos.get("stop_loss", 0.0) or 0.0)
        if current_sl > 0 and ltp is not None:
            side = self.pos.get("type") or self.pos.get("option_type") or ""
            # Normalize: in this codebase, pos["type"] is typically BUY/SELL for options
            if str(side).upper() in ("BUY", "CE"):
                return float(ltp) < current_sl
            if str(side).upper() in ("SELL", "PE"):
                return float(ltp) > current_sl
        return False

    def vol_contracted(self, atr_now):
        """Layer 4: Volatility Crush (ATR reduction)."""
        atr_e = float(self.pos.get("_atr_entry", 0.0) or 0.0)
        if atr_e > 0 and atr_now and float(atr_now) > 0:
            return float(atr_now) < atr_e * 0.7
        return False
    ...
    def time_exit(self):
        ...
        theta_e = abs(float(self.pos.get("_theta_entry", 0.0) or 0.0))
    ...
    def profit_decision(self, ltp, underlying_ltp_now):
        ...
        levels = self.pos.get("_profit_levels", {}) or {}
        if not levels:
            return None

        entry_p = float(self.pos.get("_entry_price", 0.0) or 0.0)
        entry_u = float(self.pos.get("_underlying_entry_price", 0.0) or 0.0)
        delta_e = float(self.pos.get("_delta_entry", 0.0) or 0.0)
        side = str(self.pos.get("option_type", self.pos.get("type", "CE"))).upper()

        if entry_u > 0 and abs(delta_e) > 1e-6 and underlying_ltp_now:
            spot_move = float(underlying_ltp_now) - entry_u
            expected_option_move = spot_move * abs(delta_e)
            delta_target = entry_p + expected_option_move

            if ltp >= delta_target and ltp > entry_p * 1.10:
                if ltp >= delta_target * 1.2:
                    return self.EXIT_PROFIT_L3
                if ltp >= delta_target * 1.1:
                    return self.EXIT_PROFIT_L2
                return self.EXIT_PROFIT_L1

        if ltp >= levels.get("L3", float("inf")):
            return self.EXIT_PROFIT_L3
        if ltp >= levels.get("L2", float("inf")):
            return self.EXIT_PROFIT_L2
        if ltp >= levels.get("L1", float("inf")):
            return self.EXIT_PROFIT_L1

        return None

    def is_expiry_day(self):
        expiry_str = self.pos.get("expiry", "") or ""
        if not expiry_str:
            return False
        try:
            import pandas as pd
    ...
    def evaluate(self, ltp: float, greeks_now: dict, atr_now: float,
                 underlying_ltp_now: float = 0.0):
        """
        Master RAES decision.
        Returns: (reason, exit_qty_pct)
        """
        exited_pct = float(self.pos.get("_exited_pct_total", 0.0) or 0.0)
        is_expiry = bool(self.is_expiry_day() or self.pos.get("_is_expiry_day", False))

        # Layer 2: Greeks health
        if not self.greeks_health_ok(ltp, greeks_now):
            return self.EXIT_GREEK_HEALTH, 1.0

        # Layer 3: Trend break
        if self.trend_broken(ltp):
            return self.EXIT_TREND_BREAK, 1.0

        # Layer 4: Vol crush
        if self.vol_contracted(atr_now):
            return self.EXIT_VOL_CRUSH, 1.0

        # Layer 5: Time exit
        time_hit = self.time_exit()
        if time_hit and not is_expiry:
            return self.EXIT_TIME_DECAY, 1.0

        # Layer 6: Profit decision
        profit_signal = self.profit_decision(ltp, underlying_ltp_now)
        if profit_signal and profit_signal != "NONE":
            if is_expiry:
                if profit_signal == self.EXIT_PROFIT_L1:
                    if exited_pct < 0.33 - 1e-6:
                        return self.EXIT_PROFIT_L1, 0.33
                    return "NONE", 0.0
                if profit_signal == self.EXIT_PROFIT_L2:
                    if exited_pct < 0.67 - 1e-6:
                        return self.EXIT_PROFIT_L2, 0.67
                    return "NONE", 0.0
                if profit_signal == self.EXIT_PROFIT_L3:
                    # Bring cumulative exit to 75%
                    if exited_pct < 0.75 - 1e-6:
                        return self.EXIT_PROFIT_L3, 0.75
                    return "NONE", 0.0
                if profit_signal == self.EXIT_PROFIT_L4:
                    return self.EXIT_PROFIT_L4, 1.0
                return "NONE", 0.0
            else:
                if profit_signal == self.EXIT_PROFIT_L1 and exited_pct < 0.25 - 1e-6:
                    return self.EXIT_PROFIT_L1, 0.25
                if profit_signal == self.EXIT_PROFIT_L2 and exited_pct < 0.50 - 1e-6:
                    return self.EXIT_PROFIT_L2, 0.50
                if profit_signal == self.EXIT_PROFIT_L3:
                    # Bring cumulative exit to 75%
                    if exited_pct < 0.75 - 1e-6:
                        return self.EXIT_PROFIT_L3, 0.75
...
# =============================================================================
# UNIFIED STOP LOSS MANAGER (V13 FEATURE)
# =============================================================================
class UnifiedStopLossManager:
    ...
    def __init__(self, order_manager):
        ...
        self.log = getattr(order_manager, "log", print)
        self.symbol = getattr(order_manager, "symbol", "")

        self.INITIAL_SL_ATR = 1.2
        self.EMERGENCY_SL_MULTIPLIER = 1.2
        self.MAX_LOSS_PERCENT = 0.015  # 1.5%
        self.BID_ASK_BUFFER = 0.10
        self.FLASH_CRASH_THRESHOLD = None  # disabled (was 300 in 2s)

    def check_stop_loss(self, ltp, atr=None):
        """
        Check ALL stop loss conditions
        Returns: (should_exit, reason, loss_amount_rupees)
        """
        self.position = getattr(self.om, "position", None)
        ...
        if entry is None or current_sl is None or ltp is None:
            return False, None, 0.0

        # CHECK 1: Regular SL
        sl_with_buffer = self._apply_buffer(current_sl, side)
        if self._is_sl_hit(float(ltp), sl_with_buffer, side):
            loss_rupees = self._calculate_loss(entry, float(ltp), side) * self.om.pnl_multiplier
            return True, f"Regular SL Hit (₹{current_sl:.2f})", float(loss_rupees)

        # CHECK 2: Emergency SL (risk based)
        if r_mult and r_mult > 0:
            if side in ("BUY", "CE"):
                current_pnl = (float(ltp) - entry) * self.om.pnl_multiplier
            else:
                current_pnl = (entry - float(ltp)) * self.om.pnl_multiplier
            max_loss_allowed = r_mult * self.om.pnl_multiplier * self.EMERGENCY_SL_MULTIPLIER
            if current_pnl < -max_loss_allowed:
                return True, f"Emergency SL (Loss > {self.EMERGENCY_SL_MULTIPLIER}x risk)", abs(float(current_pnl))

        # CHECK 3: Max loss percent
        if entry > 0:
            if side in ("BUY", "CE"):
                loss_pct = abs((entry - float(ltp)) / entry * 100.0)
            else:
                loss_pct = abs((float(ltp) - entry) / entry * 100.0)
            if loss_pct > self.MAX_LOSS_PERCENT * 100.0:
                loss_rupees = self._calculate_loss(entry, float(ltp), side) * self.om.pnl_multiplier
                return True, f"Max Loss % Exceeded ({loss_pct:.2f}%)", float(loss_rupees)

        # CHECK 4: Flash crash
        flash_triggered, flash_loss = self._check_flash_crash(float(ltp), entry, side)
        if flash_triggered:
            return True, f"Flash Crash Protection (₹{flash_loss:.0f} in 2s)", float(flash_loss)
    ...
    def _calculate_loss(self, entry, ltp, side):
        if side in ("BUY", "CE"):
            ...
            profit = entry - ltp
        return -profit  # positive loss when losing

    def _check_flash_crash(self, ltp: float, now_ts: float) -> bool:
        # Disabled: flash-crash heuristic was generating false exits in low-premium options
        return False, 0.0
...
# =============================================================================
# UNIFIED TRAILING PROFIT MANAGER (V13 FEATURE)
# =============================================================================
class UnifiedTrailingProfitManager:
    ...
    def update_trailing_stops(self, ltp, atr, pivot_data=None):
        ...
        """
        self.position = getattr(self.om, "position", None) or {}
        ...
        try:
            self.log(
                ...
                f"  LTP: ₹{float(ltp):.2f}\n"
                f"  ATR: {float(atr):.2f}\n"
                f"  Position type: {self.position.get('type')}\n"
                f"  Entry: ₹{self._f(self.position.get('entry_price'), 0.0):.2f}\n"
                f"{'=' * 60}",
                False
            )
        except Exception:
        ...
        entry = self._f(self.position.get("entry_price"))
        side = str(self.position.get("type", "")).upper()
        ...
        profit_rupees = profit_points * self.om.pnl_multiplier

        if profit_rupees >= 2000 and atr > 0:
            self._update_ultra_tight(ltp, profit_rupees, side)

        if atr > 0 and profit_rupees >= 0:
            self._update_tiered(ltp, atr, profit_rupees, side, entry)

        if pivot_data and atr > 0:
            self._update_cpr_dynamic(ltp, atr, pivot_data, side)

        return self._check_profit_protection(profit_rupees)
    ...
    def _update_ultra_tight(self, ltp, profit_rupees, side):
        ...
        trail_points = trail_rupees / 250.0
        ...
        if should_update:
            ...
            try:
                self.log(
                    f"[SIGNAL] [ULTRA-TIGHT] SL updated Old={current_sl:.2f} → New={new_sl:.2f} "
                    f"| Trail=₹{trail_rupees} | Locked=₹{locked:.0f}",
                    False
                )
            except Exception:
                pass
    ...
    def _update_tiered(self, ltp, atr, profit_rupees, side, entry):
        ...
        if 0 < profit_rupees < 500:
            if profit_rupees >= 500 and not self.position.get("breakeven_set"):
    ...
    def _check_profit_protection(self, profit_rupees):
        max_profit = float(self.position.get("_max_profit", 0.0) or 0.0)
        if profit_rupees > max_profit:
            self.position["_max_profit"] = float(profit_rupees)
            max_profit = float(profit_rupees)

        if max_profit <= 0:
            return False, None

        drawdown = max_profit - float(profit_rupees)
        drawdown_pct = (drawdown / max_profit * 100.0) if max_profit > 0 else 0.0
...
# =============================================================================
# MARKET REGIME DETECTION (V13 FEATURE)
# =============================================================================
class MarketRegimeDetector:
    ...
    def detect(self, ohlc_df, indicators):
        if ohlc_df is None or len(ohlc_df) < 20:
            return "UNKNOWN", 0.0

        try:
            adx = self._f(indicators.get("adx"), 0.0)
            bb_bandwidth = self._f(indicators.get("bb_bandwidth"), 0.0)
            atr = self._f(indicators.get("ATR"), 0.0)
            volume_ratio = self._f(indicators.get("volume_ratio"), 1.0)

            recent = ohlc_df.iloc[-20:]
            high_low_range = (recent['High'].max() - recent['Low'].min()) / max(1e-9, recent['Close'].mean())
            ...
            if ema5 is not None and ema21 is not None and ema50 is not None:
                if ema5 > ema21 > ema50:
                    trending_score += 0.3
                elif ema5 < ema21 < ema50:
                    trending_score += 0.3
                elif ema5 > ema21 or ema5 < ema21:
                    trending_score += 0.2
            ...
            if trending_score >= 0.7:
                try:
                    self.log(f"[REGIME] TRENDING detected (score: {trending_score:.2f})", False)
                except Exception:
                    pass
            ...
            if adx < 20:
                choppy_score += 0.4
            if bb_bandwidth < 0.005:
                choppy_score += 0.3
            if high_low_range < 0.02:
                choppy_score += 0.2
            if volume_ratio < 0.8:
                choppy_score += 0.1
            ...
            if choppy_score >= 0.6:
                try:
                    self.log(f"[REGIME] CHOPPY detected (score: {choppy_score:.2f})", False)
                except Exception:
                    pass
            ...
            if volatile_score >= 0.6:
                try:
                    self.log(f"[REGIME] VOLATILE detected (score: {volatile_score:.2f})", False)
                except Exception:
                    pass
...
# ============================================================================
try:
    # Some pandas builds expose no SettingWithCopyWarning on pd.errors.
    # Define a compatibility alias so downstream imports don't fail.
    if not hasattr(pd.errors, "SettingWithCopyWarning"):
        class _CompatSettingWithCopyWarning(Warning):
            pass
        pd.errors.SettingWithCopyWarning = _CompatSettingWithCopyWarning  # type: ignore[attr-defined]
...
# =============================================================================
# ✅ FINAL CHECKLIST CONFIG (STRONG TREND + ADAPTIVE SL/TP/RR TRAILING)
# =============================================================================
CONFIG = {
    # Indicators
    'ATR_PERIOD': 14,
    ...
    # Stop Loss - ADAPTIVE
    'INITIAL_SL_ATR_MULT': 1.3,       # Entry SL width (NORMAL + STRONG)
    'SWING_LOW_MULT': 0.5,            # Swing low backup adjustment

    # Trailing Stop Width (after RR>=1.0)
    'STRONG_TRAIL_ATR_MULT': 2.2,     # STRONG trend trail width (very wide)
    'NORMAL_TRAIL_ATR_MULT': 1.5,     # NORMAL trend trail width (medium)

    # Profit Targets
    'NORMAL_TARGET_PCT': 0.02,        # +2%
    'STRONG_TARGET_PCT': 0.05,        # +5%

    # Risk/Reward
    'RR_THRESHOLD': 1.0,              # Only trail after 1:1 RR

    # Strong Trend Detection
    'STRONG_TREND_MIN_SCORE': 83,     # >=83% => STRONG (5/6)
    'NORMAL_ENTRY_MIN_CONDITIONS': 4, # 4+ out of 5
    ...
    # Safety / session
    'ENABLE_CHECKLIST_MODE_FOR_OPTIONS': True,  # NEW: enforce checklist exits/trailing for option positions
    'ENABLE_STRONG_TREND_UPGRADE': True,        # NEW: upgrade entries to STRONG when all 6 conditions met
    'ENABLE_STRONG_AUTOBUY': False,             # Optional: pure strong-trend auto entries (kept OFF by default)

    # Daily loss behavior (NO DAY-LIMIT BLOCKING)
    'DAILY_LOSS_LIMIT_RUPEES': 0,               # informational tracking only (0 = disabled)
    'HALT_AFTER_LOSS_ONLY_IF_TRADES_LT': 3,     # If daily loss limit is set, only halt new entries before 3 trades
}

# =============================================================================
# ✅ CHECKLIST HELPERS (STRONG/NORMAL detection + adaptive SL/TP + RR trailing)
# =============================================================================
def checklist_trade_mode_from_inds(inds: Dict[str, Any]) -> Tuple[str, float, Dict[str, bool]]:
    """Return (trade_mode, score_pct, checks_dict). STRONG requires >=83% (5/6)."""
    ema20 = float(inds.get("ema_20", 0.0) or 0.0)
    ema50 = float(inds.get("ema_50", 0.0) or 0.0)
    ao = float(inds.get("ao", 0.0) or 0.0)
    ao_prev = float(inds.get("ao_prev", 0.0) or 0.0)
    atr = float(inds.get("atr", 0.0) or 0.0)
    atr_med = inds.get("atr_median20", None)
    try:
        atr_med = float(atr_med) if atr_med is not None and not pd.isna(atr_med) else None
    except Exception:
        atr_med = None

    higher_highs_5 = float(inds.get("higher_highs_5", 0.0) or 0.0)
    green_candles_5 = float(inds.get("green_candles_5", 0.0) or 0.0)

    checks = {
        "ema_alignment": ema20 > ema50,
        "ao_positive": ao > 0,
        "ao_rising": ao > ao_prev,
        "atr_expanding": (atr_med is not None) and (atr > atr_med),
        "structure": higher_highs_5 >= 3,
        "candles": green_candles_5 >= 4,
    }
    true_count = sum(1 for v in checks.values() if v)
    score = (true_count / 6.0) * 100.0

    trade_mode = "STRONG" if score >= CONFIG["STRONG_TREND_MIN_SCORE"] else "NORMAL"
    return trade_mode, score, checks


def checklist_normal_entry_ok(inds: Dict[str, Any]) -> Tuple[bool, int, Dict[str, bool]]:
    """NORMAL entry requires 4+ out of 5 (and must align with EMA+AO gates)."""
    ema20 = float(inds.get("ema_20", 0.0) or 0.0)
    ema50 = float(inds.get("ema_50", 0.0) or 0.0)
    ao = float(inds.get("ao", 0.0) or 0.0)
    close = float(inds.get("close", 0.0) or 0.0)
    momentum = float(inds.get("momentum", 0.0) or 0.0)
    atr = float(inds.get("atr", 0.0) or 0.0)
    atr_med = inds.get("atr_median20", None)
    try:
        atr_med = float(atr_med) if atr_med is not None and not pd.isna(atr_med) else None
    ...
    conds = {
        ...
        "atr_ok": (atr_med is None) or (atr >= atr_med),
    }
    true_count = sum(1 for v in conds.values() if v)
    return true_count >= CONFIG["NORMAL_ENTRY_MIN_CONDITIONS"], true_count, conds


def checklist_initial_stop(entry: float, atr: float, swing_low_5: Optional[float]) -> float:
    """Initial SL = wider of ATR-based and swing-low backup."""
    sl_atr = entry - (atr * CONFIG["INITIAL_SL_ATR_MULT"])
    sl_swing = None
    if swing_low_5 is not None and not pd.isna(swing_low_5):
        sl_swing = float(swing_low_5) - (atr * CONFIG["SWING_LOW_MULT"])
    if sl_swing is None:
        return sl_atr
    return max(sl_atr, sl_swing)  # wider (more breathing room)


def checklist_profit_target(entry: float, trade_mode: str) -> float:
    pct = CONFIG["STRONG_TARGET_PCT"] if trade_mode == "STRONG" else CONFIG["NORMAL_TARGET_PCT"]
    return entry * (1.0 + pct)


def checklist_rr(entry: float, stop_loss: float, max_price: float) -> float:
    risk = max(entry - stop_loss, 1e-9)
    reward = max(max_price - entry, 0.0)
    return reward / risk
...
class OptionBuySignal:
    ...
    # ✅ NEW (CHECKLIST MODE)
    trade_mode: str = "NORMAL"  # "NORMAL" or "STRONG"
    strength_score: float = 0.0
    strong_checks: Dict[str, bool] = field(default_factory=dict)
...
class SensexOptionBuyer:
    ...
    def parse_option_chain_to_strikes(self, chain_resp: Dict) -> Dict[float, ChainStrikeRow]:
        ...
        oi_field_debug_count = 0

        def _first_present(row_obj: Dict[str, Any], keys: Tuple[str, ...]) -> Any:
            for k in keys:
                if k in row_obj and row_obj.get(k) not in (None, ""):
                    return row_obj.get(k)
            return None

        def _normalize_option_type(row_obj: Dict[str, Any]) -> Optional[str]:
            raw = (
                row_obj.get("option_type")
                or row_obj.get("optionType")
                or row_obj.get("right")
                or row_obj.get("type")
            )
            if raw is None:
                return None
            ...
            if t in ("CE", "PE"):
                return t
            if t in ("CALL", "C") or t.startswith("CAL"):
                return "CE"
            if t in ("PUT", "P") or t.startswith("PUT"):
                return "PE"
            return None
        
        for row in chain:
            try:
                if oi_field_debug_count < 3:
                    self.log(f"[OI-FIELDS] keys={list(row.keys())}", True)
                    self.log(
                        f"[OI-FIELDS] oi={row.get('oi')}, open_interest={row.get('open_interest')}, openInterest={row.get('openInterest')}",
                        True,
                    )
                    self.log(
                        f"[OI-FIELDS] oich={row.get('oich')}, oi_chg={row.get('oi_chg')}, oi_change={row.get('oi_change')}, changeinopeninterest={row.get('changeinopeninterest')}",
                        True,
                    )
                    oi_field_debug_count += 1

                sp = self._safe_float(
                    _first_present(row, ("strike_price", "strike", "sp")),
                    None
                )
                if sp is None:
                    continue
                
                opt_type = _normalize_option_type(row)
                oi = self._safe_float(_first_present(row, ("oi", "open_interest", "openInterest")))
                oi_chg = self._safe_float(
                    _first_present(
                        row,
                        (
                            "oich",
                ...
                sym = _first_present(row, ("symbol", "tsym", "tradingsymbol"))
                ltp = self._safe_float(_first_present(row, ("ltp", "lp", "last_price")))
                iv = self._safe_float(_first_present(row, ("iv", "implied_volatility")))
                ...
                if opt_type == "CE":
                    ...
                    strikes[sp].ce_iv = iv
                    self.log(f"[OI-DEBUG] Strike {sp} CE: OI={oi}, OI_Chg={oi_chg}", True)
                elif opt_type == "PE":
                    strikes[sp].pe_oi = oi
                    ...
                    strikes[sp].pe_iv = iv
                    self.log(f"[OI-DEBUG] Strike {sp} PE: OI={oi}, OI_Chg={oi_chg}", True)
...
def compute_oi_support_resistance(self, strikes: Dict[float, ChainStrikeRow], spot_ltp: float) -> Dict[str, Any]:
    """Compute OI-based Support/Resistance from option chain.

    Rules:
    - Support  = strike with MAX Put OI (PE_OI)
    - Resistance = strike with MAX Call OI (CE_OI)
    - Also returns second levels and total PCR for context
    """
    if not strikes:
        return {}
    rows = []
    total_ce = 0.0
    total_pe = 0.0
    for sp, r in strikes.items():
        ce = float(r.ce_oi or 0.0)
        pe = float(r.pe_oi or 0.0)
        ce_chg = float(r.ce_oi_chg or 0.0) if r.ce_oi_chg is not None else 0.0
        pe_chg = float(r.pe_oi_chg or 0.0) if r.pe_oi_chg is not None else 0.0
        total_ce += ce
        total_pe += pe
        rows.append((sp, ce, pe, ce_chg, pe_chg))

    # sort for levels
    top_pe = sorted(rows, key=lambda x: x[2], reverse=True)
    top_ce = sorted(rows, key=lambda x: x[1], reverse=True)

    support = top_pe[0][0] if top_pe else None
    resistance = top_ce[0][0] if top_ce else None
    support2 = top_pe[1][0] if len(top_pe) > 1 else None
    resistance2 = top_ce[1][0] if len(top_ce) > 1 else None

    # strength using OI change (if available)
    sup_chg = top_pe[0][4] if top_pe else 0.0
    res_chg = top_ce[0][3] if top_ce else 0.0

    pcr_total = (total_pe / total_ce) if total_ce > 0 else None

    return {
        "spot": float(spot_ltp or 0.0),
        "support": float(support) if support is not None else None,
        "support2": float(support2) if support2 is not None else None,
        "resistance": float(resistance) if resistance is not None else None,
        "resistance2": float(resistance2) if resistance2 is not None else None,
        "support_oi": float(top_pe[0][2]) if top_pe else 0.0,
        "resistance_oi": float(top_ce[0][1]) if top_ce else 0.0,
        "support_oi_chg": float(sup_chg),
        "resistance_oi_chg": float(res_chg),
        "pcr_total": float(pcr_total) if pcr_total is not None else None,
    }
    ...
    def evaluate_call_buy_conditions(
        ...
        signal.atr = atr
        # attach OI support/resistance snapshot for risk adjustment
        try:
            signal.oi_sr = oi_sr  # set in analyze_and_get_signal scope
            signal.spot = spot_ltp
            signal.fut = fut_ltp
        except Exception:
            pass
        # attach OI support/resistance snapshot for risk adjustment
        ...
        # 8) OI change filters (strict, data must be present when enabled)
        if self.config["require_put_oi_support"]:
            if chain_row.pe_oi_chg is None:
                signal.reasons = ["Put OI change data unavailable - REJECTED"]
                return signal
        ...
        if self.config["require_call_oi_shortcover"]:
            if chain_row.ce_oi_chg is None:
                signal.reasons = ["Call OI change data unavailable - REJECTED"]
                return signal
    ...
    def evaluate_put_buy_conditions(
        ...
        if pcr is None or pcr > self.config["strike_pcr_max"]:
            ...
            signal.reasons = [f"Strike PCR not supportive for PE: {pcr_text} (max {self.config['strike_pcr_max']})"]
            return signal
        reasons.append(f"[OK] Strike PCR OK ({pcr:.2f})")
    ...
    def analyze_and_get_signal(self) -> OptionBuySignal:
        ...
        try:
            ...
            # OI-based Support/Resistance (from chain OI)
            oi_sr = self.compute_oi_support_resistance(strikes, spot_ltp)
            if oi_sr:
                self.log(
                    f"[OI-SR] Support={oi_sr.get('support')} (ΔOI={oi_sr.get('support_oi_chg')}) | "
                    f"Resistance={oi_sr.get('resistance')} (ΔOI={oi_sr.get('resistance_oi_chg')}) | "
                    f"PCR_total={oi_sr.get('pcr_total')}",
                    False,
                )
            ...
            test_row = strikes[candidates[0]] if candidates else None
            if test_row:
                self.log(f"[OI-TEST] CE_OI={test_row.ce_oi}, CE_OI_Chg={test_row.ce_oi_chg}", False)
                self.log(f"[OI-TEST] PE_OI={test_row.pe_oi}, PE_OI_Chg={test_row.pe_oi_chg}", False)
...
class OptionOrderManager:
    ...
    def __init__(self, trading_bot, log_fn):
        ...
        self.max_option_trades_per_day = 6
        ...
        # Rolling-to-ATM module (OFF by default)
        self.ENABLE_ROLL_TO_ATM = bool(getattr(trading_bot, "ENABLE_ROLL_TO_ATM", False))
        self.ROLL_DELTA_ABS = float(getattr(trading_bot, "ROLL_DELTA_ABS", 0.70))
        self.ROLL_MIN_PROFIT_POINTS = float(getattr(trading_bot, "ROLL_MIN_PROFIT_POINTS", 8.0))
        self.ROLL_COOLDOWN_MINUTES = int(getattr(trading_bot, "ROLL_COOLDOWN_MINUTES", 10))
    ...
    def analyze_and_trade(self) -> Optional[OptionBuySignal]:
        ...
        if self.in_option_trade and self.active_option_order_manager:
            return self._manage_active_trade()
                ...
                # Avoid first noisy minutes: do not open NEW option trades before 09:25 IST
        try:
            now_ist = dt.datetime.now(self.IST)
            if now_ist.time() < dt_time(9, 25) and ("SENSEX" in str(getattr(self.bot, 'current_symbol', '')) or "SENSEX" in str(getattr(self.bot, 'symbol', ''))):
                self.log(f"[TIME-GATE] {now_ist.strftime('%H:%M')} < 09:25 IST - skipping new entries", False)
                return None
        except Exception:
            pass
...
# Get signal from SensexOptionBuyer
    ...
    def _is_trading_halted(self) -> bool:
        ...
        # Check daily loss limit (✅ NO DAY-LIMIT BLOCKING)
        # We only halt *early* if loss limit is configured AND fewer than N trades taken.
        # After N trades, we continue trading to not miss afternoon breakouts (e.g., 14:20).
        if self.option_daily_loss_limit and self.option_daily_loss_limit > 0:
            if self.option_daily_pnl <= -self.option_daily_loss_limit and self.option_trades_today < CONFIG["HALT_AFTER_LOSS_ONLY_IF_TRADES_LT"]:
                self.log(
                    f"[ALERT] [OPT-MGR] Trading halted early (loss limit reached before {CONFIG['HALT_AFTER_LOSS_ONLY_IF_TRADES_LT']} trades)\n"
                    f"  Loss: ₹{self.option_daily_pnl:.0f} / ₹{self.option_daily_loss_limit} | Trades: {self.option_trades_today}",
                    False
                )
                return True
            elif self.option_daily_pnl <= -self.option_daily_loss_limit:
                # Informational log only, do NOT block further entries
                self.log(
                    f"[INFO] [OPT-MGR] Daily loss limit breached but trading continues (no afternoon blocking)\n"
                    f"  Loss: ₹{self.option_daily_pnl:.0f} / ₹{self.option_daily_loss_limit} | Trades: {self.option_trades_today}",
                    False
                )
    ...
    def _execute_entry(self, signal: OptionBuySignal) -> Optional[OptionBuySignal]:
        ...
        try:
            ...
            # ------------------------------------------------------------------
            # ✅ CHECKLIST MODE UPGRADE (NORMAL vs STRONG) using OPTION OHLC indicators
            # This keeps your existing signal engine intact, and only ENHANCES execution.
            # ------------------------------------------------------------------
            try:
                if CONFIG.get("ENABLE_CHECKLIST_MODE_FOR_OPTIONS", True):
                    # Pull enough 5-min candles for stable EMA/AO/ATR (>= 50 bars)
                    ohlc_opt = self.bot.fetch_ohlc(signal.symbol, tf_min=5, lookback_min=3000)
                    inds_opt = self.bot.indicator_calculator.calculate_indicators(
                        signal.symbol, "5", pivot_data=None, ohlc_df=ohlc_opt
                    )

                    # Determine STRONG/NORMAL
                    trade_mode, score_pct, checks = checklist_trade_mode_from_inds(inds_opt)

                    # Mandatory gates for ALL entries
                    gate_ok = checks.get("ema_alignment", False) and checks.get("ao_positive", False) and checks.get("atr_expanding", False)
                    if not gate_ok:
                        self.log(
                            f"[SKIP] [CHECKLIST] Entry gated out for {signal.symbol} | "
                            f"EMA20>EMA50={checks.get('ema_alignment')} AO>0={checks.get('ao_positive')} ATR>MED={checks.get('atr_expanding')}",
                            False
                        )
                        return None

                    # NORMAL must satisfy 4/5 conditions
                    if trade_mode != "STRONG":
                        ok, cnt, conds = checklist_normal_entry_ok(inds_opt)
                        if not ok:
                            self.log(
                                f"[SKIP] [CHECKLIST] Normal entry conditions not met ({cnt}/5) for {signal.symbol} | {conds}",
                                False
                            )
                            return None

                    # Use OPTION ATR for SL/TP sizing (fallback if missing)
                    opt_atr = float(inds_opt.get("atr", signal.atr or 0.0) or 0.0)
                    if not opt_atr or opt_atr <= 0:
                        opt_atr = float(signal.atr or 50.0)

                    swing_low_5 = None
                    try:
                        if ohlc_opt is not None and not ohlc_opt.empty and "Low" in ohlc_opt.columns:
                            swing_low_5 = float(ohlc_opt["Low"].tail(5).min())
                    except Exception:
                        swing_low_5 = None

                    # Build adaptive SL/TP
                    sl_price = checklist_initial_stop(signal.entry_price, opt_atr, swing_low_5)
                    tp_price = checklist_profit_target(signal.entry_price, trade_mode)

                    # Apply into signal (do NOT remove old fields; just enhance)
                    signal.trade_mode = trade_mode
                    signal.strength_score = float(score_pct)
                    signal.strong_checks = checks

                    signal.atr = opt_atr
                    signal.sl_price = float(sl_price)
                    signal.tp_price = float(tp_price)
                    signal.sl_points = float(signal.entry_price - signal.sl_price)
                    signal.tp_points = float(signal.tp_price - signal.entry_price)

                    # Attach for logs / debugging
                    signal.debug_info.update({
                        "checklist_mode": True,
                        "checklist_tf": "5",
                        "strength_score_pct": float(score_pct),
                ...
                # Never break live trading due to checklist enhancements
                self.log(f"[WARN] [CHECKLIST] Failed to compute checklist indicators for {signal.symbol}: {e}", False)
    ...
    def _manage_active_trade(self) -> Optional[OptionBuySignal]:
        ...
        try:
            ...
            # Optional: Roll-to-ATM if delta becomes too ITM (OFF by default)
            if self.ENABLE_ROLL_TO_ATM and ("SENSEX" in str(self.active_signal.symbol)):
                try:
                    now_ist = dt.datetime.now(self.IST)
                    if self._last_roll_time and (now_ist - self._last_roll_time).total_seconds() < self.ROLL_COOLDOWN_MINUTES * 60:
                        pass
                    else:
                        # Only consider roll if already in decent profit (profit locked)
                        if profit_points >= self.ROLL_MIN_PROFIT_POINTS:
                            # Compute current delta safely (Black-76) using FUT price and current option LTP
                            fut_symbol = getattr(self.bot, "symbols", [None])[0] if hasattr(self.bot, "symbols") else None
                            fut_quote = self.bot.fyers_sdk_instance.quotes({"symbols": self.bot.current_symbol or "BSE:SENSEX-INDEX"})
                            fut_ltp = None
                            try:
                                if fut_quote.get("s")=="ok" and fut_quote.get("d"):
                                    fut_ltp = float(fut_quote["d"][0].get("v", {}).get("lp") or 0)
                            except Exception:
                                fut_ltp = None

                            # Parse strike + type from symbol
                            sym = str(self.active_signal.symbol)
                            m = re.search(r"(\d{4,6})(CE|PE)$", sym)
                            if m and fut_ltp:
                                strike = float(m.group(1))
                                opt_type = m.group(2)
                                # Approx time to expiry (fallback to 2 days if not available)
                                T = 2.0 / 365.0
                                try:
                                    chain = self.bot.fyers_sdk_instance.optionchain(data={"symbol": "BSE:SENSEX-INDEX", "strikecount": "2"})
                                    if chain and chain.get("s")=="ok":
                                        # best-effort parse expiry epoch or date if present
                                        exp = ((chain.get("data") or {}).get("expiryData") or [])
                                        if exp and isinstance(exp, list):
                                            # pick first expiry
                                            exp_ts = exp[0].get("expiry") or exp[0].get("expiryDate") or exp[0].get("date")
                                            # leave T default if parse fails
                                except Exception:
                                    pass

                                iv = Black76Greeks.implied_vol(fut_ltp, strike, 0.07, T, opt_type[0], float(ltp))
                                greeks = Black76Greeks.greeks_extended(fut_ltp, strike, 0.07, iv, T, opt_type[0])
                                delta = float(greeks.get("delta") or 0.0)

                                if abs(delta) >= self.ROLL_DELTA_ABS:
                                    self.log(f"[ROLL] Trigger: |delta|={abs(delta):.2f} >= {self.ROLL_DELTA_ABS} and profit={profit_points:.2f} pts. Rolling to fresh ATM.", True)
                                    # Exit current, then re-enter ATM using the option buyer engine
                                    self._last_roll_time = now_ist
                                    self._execute_exit(ltp, f"Roll trigger (|Δ|={abs(delta):.2f})")
                                    # Immediate re-entry into fresh ATM/near-ATM (same direction) if signal still valid
                                    try:
                                        new_sig = self._get_signal_for_current_symbol()
                                        if new_sig and getattr(new_sig, 'direction', None) == getattr(self.active_signal, 'direction', None):
                                            self.log(f"[ROLL] Re-entering fresh contract: {new_sig.symbol}", True)
                                            return self._execute_entry(new_sig)
                                    except Exception as _e2:
                                        self.log(f"[ROLL] Re-entry failed: {_e2}", False)
                                    return None
                except Exception as _re:
                    self.log(f"[ROLL] Error in roll check: {_re}", False)
...
def _build_ai_cpr_features(ltp: float, indicators: dict, pivot_data: dict, ohlc_df=None) -> np.ndarray:
    ...
    if ohlc_df is not None and not ohlc_df.empty and len(ohlc_df) >= 3:
        try:
            # Use local extractor to avoid cross-file pandas warning API mismatches.
...
def analyze_cpr_strategy(indicators, pivot_data, ai_predictor, ohlc_df=None):
    ...
    if ohlc_df is None or ohlc_df.empty:
        # Reuse precomputed indicator dataframe if available.
        if isinstance(indicators, dict):
            df_hint = indicators.get("df")
            if isinstance(df_hint, pd.DataFrame) and not df_hint.empty:
                ohlc_df = df_hint.copy()
        # Last fallback: direct OHLC fetch to avoid zero candle features.
        if ohlc_df is None or ohlc_df.empty:
            try:
                symbol_hint = indicators.get("symbol") if isinstance(indicators, dict) else None
                if symbol_hint:
                    ohlc_df = fetchOHLC1(symbol_hint, "5", duration=3)
            except Exception as _e:
                logger.warning(f"[AI-CPR] OHLC fallback fetch failed: {_e}")
        if ohlc_df is None or ohlc_df.empty:
            logger.warning("[AI-CPR] No OHLC data provided - candle features may be zero")
    ...
    # AI fallback: if CPR rules are neutral but AI is clear with decent confidence,
    # provide a soft directional strategy instead of "None".
    if trade_strategy == "None" and ai_dir is not None and ai_conf is not None:
        ai_conf_val = float(ai_conf)
        if ai_conf_val >= 0.30 and adx >= 20:
            if ai_dir > 0 and ema_5 >= ema_21 * 0.995:
                trade_strategy = "Buy"
                reason = f"AI-FALLBACK BUY (conf={ai_conf_val:.2f}, ADX={adx:.1f})"
            elif ai_dir < 0 and ema_5 <= ema_21 * 1.005:
                trade_strategy = "Sell"
                reason = f"AI-FALLBACK SELL (conf={ai_conf_val:.2f}, ADX={adx:.1f})"
...
class IndicatorCalculator:
    ...
    def calculate_indicators(self, symbol, timeframe, pivot_data=None, ohlc_df=None):
        if not hasattr(self, "_pivot_warned_symbols"):
            self._pivot_warned_symbols = set()
        ...
        if not pivot_data:
            ...
            if is_option:
                ...
                if symbol not in self._pivot_warned_symbols:
                    self.bot.log_message(
                        f"[WARNING] [INDICATORS] No pivot data for {symbol} - attempting emergency load",
                        False
                    )
                    self._pivot_warned_symbols.add(symbol)
                pivot_data = {}
        ...
        # ---------------------------------------------------------------------
        # ✅ CHECKLIST INDICATORS (EMA20/EMA50, AO, ATR median, structure, candles)
        # NOTE: We compute these here to keep the script fully live (no CSV needed).
        # ---------------------------------------------------------------------
        ohlc["EMA5"] = ohlc["Close"].ewm(span=5, adjust=False).mean()
        ohlc["EMA9"] = ohlc["Close"].ewm(span=9, adjust=False).mean()
        ohlc["EMA21"] = ohlc["Close"].ewm(span=21, adjust=False).mean()
        ohlc["EMA50"] = ohlc["Close"].ewm(span=50, adjust=False).mean()
        ohlc["EMA200"] = ohlc["Close"].ewm(span=200, adjust=False).mean()

        # Awesome Oscillator (AO) = SMA(5, median_price) - SMA(34, median_price)
        median_price = (ohlc["High"] + ohlc["Low"]) / 2.0
        ao_fast = median_price.rolling(CONFIG["AO_FAST"]).mean()
        ao_slow = median_price.rolling(CONFIG["AO_SLOW"]).mean()
        ohlc["AO"] = ao_fast - ao_slow

        # ATR median filter (quiet-period avoidance gate)
        if "ATR" in ohlc.columns:
            ohlc["ATR_MED20"] = ohlc["ATR"].rolling(CONFIG["ATR_MEDIAN_WINDOW"]).median()
        else:
            ohlc["ATR_MED20"] = np.nan

        # Structure & candle quality
        ohlc["higher_high"] = ohlc["High"] > ohlc["High"].shift(1)
        ohlc["green_candle"] = ohlc["Close"] > ohlc["Open"]
        ohlc["higher_highs_5"] = ohlc["higher_high"].rolling(5).sum()
        ohlc["green_candles_5"] = ohlc["green_candle"].rolling(5).sum()

        body = (ohlc["Close"] - ohlc["Open"]).abs()
        rng = (ohlc["High"] - ohlc["Low"]).replace(0, np.nan)
        ohlc["body_ratio"] = (body / rng).fillna(0.0)
        ...
        )        # --- SuperTrend direction (for hard gating & trend decision) ---
        st14_dir = 0
        st21_dir = 0
        try:
            # Get OHLC data for SuperTrend calculation
            df = self.bot.fetch_ohlc(symbol, timeframe, 60) if hasattr(self.bot, 'fetch_ohlc') else ohlc_df
            if df is None or df.empty:
                df = ohlc_df
            
            if df is not None and not df.empty and len(df) >= 21:
                _, _, _, tr14 = supertrend(df[["High", "Low", "Close"]].copy(), period=14, multiplier=2)
                _, _, _, tr21 = supertrend(df[["High", "Low", "Close"]].copy(), period=21, multiplier=1)
                
                if tr14 is not None and len(tr14) > 0:
                    _v = tr14.iloc[-1]
                    st14_dir = int(_v) if _v == _v else 0
                if tr21 is not None and len(tr21) > 0:
                    _v = tr21.iloc[-1]
                    st21_dir = int(_v) if _v == _v else 0
        except Exception as e:
            if hasattr(self.bot, 'log_message'):
                self.bot.log_message(f"[SUPER-TREND] Error calculating SuperTrend: {e}", True)
            else:
                print(f"[SUPER-TREND] Error calculating SuperTrend: {e}")
            st14_dir = 0
            st21_dir = 0
        ...
        return {
            ...
            "ao": float(latest.get("AO", 0.0)),
            "ao_prev": float(prev.get("AO", 0.0)),
            "atr_median20": float(latest.get("ATR_MED20", np.nan)) if not pd.isna(latest.get("ATR_MED20", np.nan)) else float("nan"),
            "higher_highs_5": float(latest.get("higher_highs_5", 0.0)),
            "green_candles_5": float(latest.get("green_candles_5", 0.0)),
            "body_ratio": float(latest.get("body_ratio", 0.0)),
            "higher_high": bool(latest.get("higher_high", False)),
            "close_prev": float(prev.get("Close", np.nan)) if "Close" in prev else float("nan"),
...
class OrderManager:
    ...
    def __init__(self, fyers_service, symbol, lot_size, log_fn, state_path, ai_predictor: CPR_AIPredictor, event_log=None, bot=None, is_option=False, point_value=None, **_):
        ...
        # Point value / P&L multiplier mapping
        # NOTE: Order quantity (lot_size) and P&L conversion multiplier are NOT the same thing.
        # `self.lot` is used for order quantity.
        # `self.pnl_multiplier` is used to convert price points -> rupees for *one trade*.
        #
        # This fixes the historical bug where scripts hardcoded `* 250` everywhere (SENSEX-only),
        # which breaks NATGASMINI/NIFTY P&L and SL/TP logic.
        self.point_value = float(point_value) if point_value is not None else 1.0

        self.pnl_multiplier = get_pnl_multiplier(symbol, is_option=is_option, lot_qty=self.lot)
        ...
        # [OK] NEW: Circuit breaker (loss streak) tracking
        self.max_consecutive_losses = 3
        self.circuit_breaker_loss_threshold = 300  # ₹300 across last 3 losing trades
        self.consecutive_losses = 0
        self.recent_trade_pnls = []  # keep last N trade PnLs (rupees)
        ...
        # =============================================================================
        # V13 INTEGRATION: Initialize new managers
        # =============================================================================
        ...
        if not hasattr(self, 'regime_detector'):
            self.regime_detector = MarketRegimeDetector(self.log)
            self.log("[INIT] Market Regime Detector initialized", False)

        if not hasattr(self, 'hybrid_target_system'):
            self.hybrid_target_system = HybridProfitTargetSystem(self.log)
            self.log("[INIT] Hybrid Target System initialized", False)

        if not hasattr(self, 'greeks_scorer'):
            self.greeks_scorer = GreeksHeatmapScorerV7()
            self.log("[INIT] Greeks Heatmap Scorer initialized", False)
    ...
    def _save_state(self):
        # ✅ ATOMIC state save (backup + verify)
    ...
    def _save_state_atomic(self):
        """Atomic state save with .tmp write, .bak backup, and verification."""
        ...
        try:
            ...
            with open(temp_path, "w", encoding="utf-8") as f:
                json.dump(convert_to_serializable(self.position), f, ensure_ascii=False, indent=2, default=str)
            ...
            if not os.path.exists(self.state_path):
                self.log("[ERROR] Atomic save verification failed: state file missing", False)
                return False
            ...
            try:
                ...
                self.log(f"[ERROR] Atomic save verification failed: {e}", False)
                return False
    ...
    def _process_entry(self, side, reason, ltp, atr, bar_key=None, indsP=None):
        ...
        if is_live_success or is_paper_override:
            ...
            # =============================================================================
            # V13: HYBRID TARGETS + GREEKS SNAPSHOT (entry-time)
            # =============================================================================
            try:
                # Collect entry-time Greeks (if available in indsP)
                g_delta = self._f(indsP.get("delta"), 0.50)
                g_gamma = self._f(indsP.get("gamma"), 0.0)
                g_theta = self._f(indsP.get("theta"), 0.0)
                g_vega  = self._f(indsP.get("vega"), 0.0)
                g_iv    = self._f(indsP.get("iv"), 0.0)

                # Underlying (index) price at entry, if available
                underlying_entry = self._f(
                    indsP.get("underlying_ltp") or indsP.get("spot") or indsP.get("underlying_price"),
                    0.0
                )

                indicators_for_targets = {
                    "delta": g_delta,
                    "gamma": g_gamma,
                    "theta": g_theta,
                    "vega": g_vega,
                    "iv": g_iv,
                    "days_to_expiry": int(indsP.get("days_to_expiry", 7) or 7),
                    "vix_entry": self._f(indsP.get("vix") or indsP.get("vix_entry"), 20.0),
                    "vix_now": self._f(indsP.get("vix") or indsP.get("vix_now"), 20.0),
                    "iv_now": g_iv,
                    "now_ist": dt.datetime.now(self.IST) if hasattr(self, "IST") else dt.datetime.now(),
                    "is_expiry_day": is_expiry_day_from_days(int(indsP.get("days_to_expiry", 7) or 7)),
                }

                if hasattr(self, "hybrid_target_system") and self.hybrid_target_system:
                    ht = self.hybrid_target_system.calculate_hybrid_targets(ltp, atr, indicators_for_targets)
                    if getattr(ht, "exit_signal", False):
                        self.log(f"[ENTRY-BLOCKED] Hybrid Target System veto: {ht.exit_reason}", False)
                        return False

                    # Override initial SL with hybrid SL if valid
                    if ht and ht.sl_price and ht.sl_price > 0:
                        initial_sl = float(ht.sl_price)

                    # Build 4-tier profit levels (premium targets)
                    tp_points = float(ht.tp_points or (2.0 * atr))
                    levels = {
                        "L1": round(ltp + 0.50 * tp_points, 2),
                        "L2": round(ltp + 0.80 * tp_points, 2),
                        "L3": round(ltp + 1.00 * tp_points, 2),
                        "L4": round(ltp + 1.20 * tp_points, 2),
                    }
                else:
                    # Fallback: ATR-based levels
                    tp_points = float(2.0 * atr)
                    levels = {
                        "L1": round(ltp + 0.50 * tp_points, 2),
                        "L2": round(ltp + 0.80 * tp_points, 2),
                        "L3": round(ltp + 1.00 * tp_points, 2),
                ...
                self.log(f"[WARN] Hybrid targets/greeks snapshot failed: {_e}", True)
                g_delta = g_gamma = g_theta = g_vega = g_iv = 0.0
            ...
            # FIX #2/#6: Persist volatility scaling + expiry flags for dynamic profit tiers/trailing
            _now_ist = dt.datetime.now(self.IST) if hasattr(self, "IST") else dt.datetime.now()
            _dte = int(indicators_for_targets.get("days_to_expiry", 7) or 7)
            _is_expiry = bool(indicators_for_targets.get("is_expiry_day") or is_expiry_day_from_days(_dte))
            _vix_now = float(indicators_for_targets.get("vix_now", 20.0) or 20.0)
            _iv_now = indicators_for_targets.get("iv_now") or indicators_for_targets.get("iv")
            try:
                _iv_now = float(_iv_now) if _iv_now is not None else None
            except Exception:
                _iv_now = None
            _vol_scale = vol_target_scale(vix_now=_vix_now, iv_now=_iv_now, is_expiry_day=_is_expiry, now_ist=_now_ist)
            ...
            pos_update = {
                ...
                "_exits_this_bar": 0,
                "qty": int(self.lot),
                "_initial_qty": int(self.lot),
                "_exited_qty": 0,
                "_exited_pct_total": 0.0,
                "_entry_price": ltp,
                "_entry_time": now_iso,
                "_atr_entry": float(atr),
                "_delta_entry": float(g_delta) if 'g_delta' in locals() else 0.0,
                "_gamma_entry": float(g_gamma) if 'g_gamma' in locals() else 0.0,
                "_theta_entry": float(g_theta) if 'g_theta' in locals() else 0.0,
                "_vega_entry": float(g_vega) if 'g_vega' in locals() else 0.0,
                "_iv_entry": float(g_iv) if 'g_iv' in locals() else 0.0,
                "_underlying_entry_price": float(underlying_entry) if 'underlying_entry' in locals() else 0.0,
                "_profit_levels": levels if 'levels' in locals() else {},
                "_profit_tier": 0,
                "_vol_scale": _vol_scale,
                "_is_expiry_day": _is_expiry,
                "_now_ist": _now_ist
            }
    ...
    def _process_exit_qty(self, reason: str, ltp: float, exit_qty: int) -> bool:
        """Exit partial quantity by placing opposite market order. Falls back to full exit."""
        try:
            if not self.position or self.position.get("type") == "FLAT":
                return True

            cur_qty = int(self.position.get("qty") or self.lot or 0)
            ...
            # Full exit path
            if exit_qty >= cur_qty:
                return self._process_exit(reason, ltp)

            side = self.position.get("type", "BUY")
            exit_side = "SELL" if side == "BUY" else "BUY"

            resp = None
            if self.PAPER_TRADING_MODE:
                self.log(f"PAPER TRADE: Simulating PARTIAL exit {exit_qty}/{cur_qty} for {self.symbol}. Reason: {reason}", False)
                resp = {"s": "ok", "code": 200, "message": "Paper Partial Exit"}
            else:
                # Place opposite market order for the exit quantity
                resp = self.svc.place_market_order(self.symbol, exit_side, exit_qty)

            if isinstance(resp, dict) and (resp.get("s") == "ok" or resp.get("code") in [200, -66, 204]):
                # Update position qty and exited pct
                new_qty = max(0, cur_qty - exit_qty)
                self.position["qty"] = new_qty
                init_qty = int(self.position.get("_initial_qty") or cur_qty)
                exited_qty = int(self.position.get("_exited_qty") or 0) + exit_qty
                self.position["_exited_qty"] = exited_qty
                self.position["_exited_pct_total"] = round(exited_qty / init_qty, 4) if init_qty > 0 else 1.0

                self.log(f"[EXIT-PARTIAL] {exit_qty}/{cur_qty} exited. Remaining qty={new_qty}. Reason: {reason}", False)

                # If fully flattened
                if new_qty <= 0:
                    return self._process_exit(reason + " (flatten)", ltp)

                self._save_state()
                return True

            self.log(f"[ERROR] Partial exit failed: {resp}", False)
            return False

        except Exception as e:
            self.log(f"[ERROR] _process_exit_qty exception: {e}", False)
            return False
    ...
    def _process_exit(self, reason, ltp):
        ...
        if isinstance(resp, dict) and (resp.get("s") == "ok" or resp.get("code") in [-66, 204]):
            ...
            if ltp_diff is not None:
                # [OK] Update P&L tracking in RUPEES (premium points × lot × point_value)
                pnl_points = float(ltp_diff)
                pnl_rupees = pnl_points * float(self.lot) * float(self.point_value)

                self.daily_loss += pnl_rupees
                self.trades_today += 1

                # Maintain recent PnL window for circuit breaker
                self.recent_trade_pnls.append(pnl_rupees)
                if len(self.recent_trade_pnls) > 20:
                    self.recent_trade_pnls = self.recent_trade_pnls[-20:]
    ...
    def check_circuit_breaker(self):
        """Emergency stop on rapid losses (no external dependencies)."""
        try:
            pnls = list(self.recent_trade_pnls or [])
            if len(pnls) < self.max_consecutive_losses:
                return False

            last_n = pnls[-self.max_consecutive_losses:]
            # Circuit triggers if all last N are losses OR total loss breaches threshold
            all_losses = all(p < 0 for p in last_n)
            total_loss = sum(p for p in last_n if p < 0)

            if all_losses or total_loss <= -abs(self.circuit_breaker_loss_threshold):
                self.log(
                    f"[ALERT] CIRCUIT BREAKER TRIGGERED\\n"
                    f"  Last {self.max_consecutive_losses} trades: {last_n}\\n"
                    f"  Total loss: ₹{total_loss:.2f} | Threshold: ₹-{abs(self.circuit_breaker_loss_threshold)}\\n"
                    f"  HALTING ALL TRADING UNTIL MANUAL REVIEW",
                    False
                )
                return True
        except Exception as e:
            self.log(f"[WARN] Circuit breaker check failed: {e}", True)
        return False
    ...
    def _check_intelligent_exit(self, ltp, inds):
        ...
        # ✅ CHECKLIST MODE manages exits separately (avoid early/legacy exits)
        if pos and pos.get("checklist_mode") and CONFIG.get("ENABLE_CHECKLIST_MODE_FOR_OPTIONS", True):
            ...
            if ema5 and close and open_price:
                ...
                if side == "SELL" and trend == "GREEN":
                    ...
                    if is_red_candle and closed_above_ema5:
                        ...
                        profit_rupees = current_profit * self.om.pnl_multiplier
        ...
        if ema5 and ltp:
            ...
            if side == "SELL" and trend == "GREEN":
                ...
                loss_rupees = current_loss * self.om.pnl_multiplier
        ...
        try:
            ...
            if side == "BUY" and aligned == "SELL" and conf >= 0.66 and max_profit > 1:
                self.log(
                    ...
                    f"  Profit: ₹{current_profit * self.om.pnl_multiplier:.0f}\n"
    ...
    def _check_trailing_profit(self, ltp, inds=None):
        ...
        # ✅ CHECKLIST MODE (STRONG/NORMAL): RR-based trailing + case-based exits
        if pos and pos.get("checklist_mode") and CONFIG.get("ENABLE_CHECKLIST_MODE_FOR_OPTIONS", True):
            return self._manage_position_checklist(ltp, inds or {})
        ...
        # Volatility scale (Fix #6): widen targets/trailing on high vol, tighten on low vol.
        vol_scale = float(pos.get("_vol_scale", 1.0) or 1.0)
        is_expiry_day = bool(pos.get("_is_expiry_day", False))
        now_ist = pos.get("_now_ist")
        ...
        # Apply volatility scaling to profit tiers
        tier_multipliers = [round(x * vol_scale, 3) for x in tier_multipliers]

        # Trailing loosen/tighten based on vol + expiry time
        if vol_scale >= 2.5:
            trailing_pct = min(0.35, trailing_pct + 0.05)
        elif vol_scale <= 1.5:
            trailing_pct = max(0.08, trailing_pct - 0.03)

        if is_expiry_day and now_ist is not None and now_ist.time() >= dt_time(14, 0):
            trailing_pct = max(0.08, trailing_pct - 0.05)
    ...
    def _should_enter_early(self, ltp, inds, ohlc_df):
        ...
        try:
            ...
            # ========================================
            # CHECK 4: VOLUME CONFIRMATION (adaptive)
            # ========================================
            volume_ratio = self._f(inds.get("volume_ratio"), 1.0)
            bb_bandwidth = self._f(inds.get("bb_bandwidth"), 0.01)
            min_vol = 0.8 if bb_bandwidth > 0.012 else 1.0
            if volume_ratio < min_vol:
                return None, 0.0, f"Low volume ({volume_ratio:.2f}x < {min_vol:.2f}x)"
    ...
    def execute_unified_strategy(self, ltp, all_inds, primary_tf=None):
        ...
        if current_pos != "FLAT":
            ...
            # =============================================================================
            # V13: GA-RAES EXIT CONTROLLER (multi-layer exits + partials)
            # =============================================================================
            try:
                # Skip if checklist mode already handles exits
                if not (self.position.get("checklist_mode") and CONFIG.get("ENABLE_CHECKLIST_MODE_FOR_OPTIONS", True)):
                    inds_now = _tf(primary_tf)
                    atr_now = self._get_atr_with_fallback(inds_now, ltp)
                    greeks_now = {
                        "delta": self._f(inds_now.get("delta")),
                        "gamma": self._f(inds_now.get("gamma")),
                        "theta": self._f(inds_now.get("theta")),
                        "vega": self._f(inds_now.get("vega")),
                        "iv": self._f(inds_now.get("iv")),
                    }

                    underlying_now = self._f(
                        inds_now.get("underlying_ltp") or inds_now.get("spot") or inds_now.get("underlying_price"),
                        0.0
                    )

                    raes = CombinedGARaes(self.position, self.log)
                    reason, exit_pct = raes.evaluate(
                        ltp=float(ltp),
                        greeks_now=greeks_now,
                        atr_now=float(atr_now or 0.0),
                        underlying_ltp_now=float(underlying_now or 0.0)
                    )

                    if reason and reason != "NONE" and exit_pct and exit_pct > 0:
                        init_qty = int(self.position.get("_initial_qty") or self.position.get("qty") or self.lot)
                        target_exit_qty = int(math.ceil(exit_pct * init_qty))
                        already_exited = int(self.position.get("_exited_qty") or 0)
                        # Exit only remaining tranche
                        tranche_qty = max(0, target_exit_qty - already_exited)

                        if tranche_qty > 0:
                            self.log(f"[GA-RAES] Exit signal: {reason} | pct={exit_pct:.2f} | tranche_qty={tranche_qty}", False)
                            self._process_exit_qty(f"GA-RAES: {reason}", ltp, tranche_qty)
                            self._save_state()
                            return
            except Exception as e:
                self.log(f"[WARN] GA-RAES evaluation failed: {e}", True)
...
# ========================================
        ...
        bb_width_ai = self._f(inds.get("bb_bandwidth"), 0.0) if isinstance(inds, dict) else 0.0
        ai_min_conf_dynamic = 0.20 if bb_width_ai > 0.015 else self.AI_MIN_CONF
        ...
        try:
            ...
            if pivot_data and "TC" in pivot_data and "BC" in pivot_data:
                ...
                self.log(
                    ...
                    f"Threshold (Min): {ai_min_conf_dynamic:.4f}\n"
                    f"Threshold (Override): 0.75\n"
                    f"Status: {'[OK] ACCEPTED' if ai_confidence_raw >= ai_min_conf_dynamic else '[ERROR] REJECTED'}",
                    False
                )
                if ai_distribution:
                ...
                elif ai_label and ai_confidence_raw >= ai_min_conf_dynamic:
                    self.log(
        ...
        try:
            ...
            if pivot_data and "TC" in pivot_data and "BC" in pivot_data:
                ...
                ai_min_conf_dynamic_2 = 0.20 if (bb_bandwidth and bb_bandwidth > 0.015) else self.AI_MIN_CONF
                self.log(
                    f"[AI-CPR] Threshold: {ai_min_conf_dynamic_2:.4f} | Status: {'[OK] ACCEPTED' if ai_confidence_raw >= ai_min_conf_dynamic_2 else '[ERROR] REJECTED'}",
                    False
                )
                if ai_distribution:
                    self.log(
                ...
                if ai_label and ai_confidence_raw >= ai_min_conf_dynamic_2:
                    ai_label_upper = str(ai_label).upper()
                    ...
                    if is_counter_trend:
                        min_confidence_required = 0.55 if (bb_bandwidth and bb_bandwidth > 0.015) else 0.70
                    else:
                        min_confidence_required = ai_min_conf_dynamic_2
                    ...
                    if ai_confidence_raw >= min_confidence_required:
                        ...
                        reason = (
                            f"counter-trend, needs {min_confidence_required:.2f}+"
                            if is_counter_trend else f"< threshold {ai_min_conf_dynamic_2:.2f}"
                        )
                        self.log(
                    ...
                    if ai_label:
                        self.log(
                            f"[AI-CPR] [ERROR] Signal rejected: confidence {ai_confidence_raw:.3f} < {ai_min_conf_dynamic_2:.3f}",
                            True
                        )
        ...
        if final_signal and current_pos == "FLAT":
            ...
            if not detected_trend or detected_trend == "None" or str(detected_trend).lower() == "none":
                ...
                if final_signal == "BUY":
                    ...
                    # Allow counter-trend with VERY strong signal
                        ...
                        is_volatile_day = bool(bb_bandwidth and bb_bandwidth > 0.015)
                        if is_volatile_day:
                            counter_trend_valid = (
                                    buy_count >= 3 and
                                    buy_score >= 2.0 and
                                    ai_in_buy and
                                    confidences["ai_cpr"] >= 0.55 and
                                    momentum_pct >= 0.4 and
                                    volume_ratio >= 0.8
                            )
                        else:
                            counter_trend_valid = (
                                    ...
                                    buy_score >= 2.5 and
                                    ai_in_buy and
                                    confidences["ai_cpr"] >= 0.65 and
                                    momentum_pct >= 0.5 and
                                    volume_ratio >= 1.0
                            )
            ...
            if final_signal:
                ...
                is_rejection = (
                        ...
                        confidences.get("rejection_candle", 0) >= 0.70
                )
                ...
                if is_rejection or is_volume_breakout:
                    ...
                    if final_signal == "BUY":
                        if bb_bandwidth and bb_bandwidth > 0.015:
                            min_momentum = 0.3
                        elif adx_val >= 30:
                            min_momentum = 0.2
                        elif adx_val >= 25:
                            ...
                            if ltp < supertrend:
                                ...
                                if sell_count >= 3:
                                    ...
                                    self.log(
                                        ...
                                        f"LTP:{ltp:.2f}, ST:{supertrend:.2f} (gap:{gap_pct:.3f}%) | "
                                        f"Only {sell_count} votes (need 3+ for override)",
                                        False
                                    )
                                    final_signal = None
                            ...
                            if ltp > supertrend:
                                ...
                                if buy_count >= 3:
                                    ...
                                    self.log(
                                        ...
                                        f"LTP:{ltp:.2f}, ST:{supertrend:.2f} (gap:{gap_pct:.3f}%) | "
                                        f"Only {buy_count} votes (need 3+ for override)",
                                        False
                                    )
                                    final_signal = None
                        ...
                        if atr_here:
                            ...
                            if current_sl and entry:
                                ...
                                if should_update:
                                    ...
                                    locked_rupees = locked_profit * self.om.pnl_multiplier
...
class UnifiedStopLossManager:
    ...
    def check_stop_loss(self, ltp, atr=None):
        ...
        if self._is_sl_hit(ltp, sl_with_buffer, side):
            loss = self._calculate_loss(entry, ltp, side) * self.om.pnl_multiplier
        ...
        if r_mult > 0:
            if side == "BUY":
                current_pnl = (ltp - entry) * self.om.pnl_multiplier
            else:  # SELL
                current_pnl = (entry - ltp) * self.om.pnl_multiplier
    ...
    def _check_flash_crash(self, ltp, entry, side):
        ...
        try:
            ...
            if time_diff <= 2:
                if side == "BUY":
                    rapid_loss = (last_ltp - ltp) * self.om.pnl_multiplier
                else:
                    rapid_loss = (ltp - last_ltp) * self.om.pnl_multiplier
            ...
            try:
                ...
                if "error" not in current_inds:
                    ...
                    ai_analysis = analyze_cpr_strategy(
                        ...
                        raw,
                        self.ai_predictor,
                        ohlc_df=current_inds.get("df") if isinstance(current_inds, dict) else None
                    )
                    ai_label_val = str(ai_analysis.get("ai_cpr_label", "None"))
                    ai_label_upper = ai_label_val.upper()
                    inferred_ai_signal = "None"
                    if "SELL" in ai_label_upper:
                        inferred_ai_signal = "Sell"
                    elif "BUY" in ai_label_upper:
                        inferred_ai_signal = "Buy"
                    ...
                    clean["ai_context"] = {
                        ...
                        "ai_signal_from_label": inferred_ai_signal,
                        "ai_confidence": ai_analysis.get("ai_confidence", 0.0),
                        "ai_label": ai_label_val
                    }

                    self.log_message(
                        f"[OK] AI Context added: strategy={ai_analysis.get('trade_strategy', 'None')} | "
                        f"label={ai_label_val} (label-signal={inferred_ai_signal}) "
...
class UnderlyingConfigV2:
    ...
    fallback_on_no_candidates: bool = True
    allow_no_greeks_fallback: bool = True
...
class FyersBrokerAdapterV2(BrokerAPIV2):
    def __init__(self, fyers, paper_trade: bool = False) -> None:
        self.fyers = fyers
...
class ATRRiskManager:
    ...
    def __init__(
        self,
        point_value: float,
        atr_sl_mult: float = 0.5,     # SL = ATR * 0.5 (tighter than 0.6)
        atr_tp_mult: float = 1.4,     # TP = ATR * 1.4 (layered runner uses DynamicTrailingSL)
        min_sl_pct: float = 0.22,     # 22% minimum SL (was 30%)
        max_sl_pct: float = 0.40      # safety cap (was 45%)
    ):
    ...
    def compute_sl_tp(
        ...
        atr_points: float,
        delta_abs: float = 0.5,
        oi_target_move: float | None = None,
        oi_stop_move: float | None = None,
        ...
        # ATR converted to option premium move using delta approximation.
        delta_abs = float(delta_abs or 0.0)
        if delta_abs <= 0:
            delta_abs = 0.5
    
        # Regime-based adjustment (OI build-up score)
        # - Strong aligned regime: widen TP and allow slightly wider SL (avoid premature stopouts on trend days)
        # - Weak/against regime: tighten SL and reduce TP
        try:
            rs = float(regime_score) if regime_score is not None else 0.0
        except Exception:
            rs = 0.0
        ot = str(opt_type or '').upper()
        dir_sign = 1 if ot == 'CE' else (-1 if ot == 'PE' else 0)
        strength = min(abs(rs) / 5.0, 1.0) if rs != 0 else 0.0
        aligned = (dir_sign != 0 and (rs * dir_sign) > 0)
    
        sl_mult = float(self.atr_sl_mult)
        tp_mult = float(self.atr_tp_mult)
        if strength > 0:
            if aligned:
                tp_mult *= (1.0 + 0.25 * strength)
                sl_mult *= (1.0 + 0.10 * strength)
            else:
                tp_mult *= (1.0 - 0.20 * strength)
                sl_mult *= (1.0 - 0.15 * strength)
    
        atr_sl_move = abs(atr_points) * sl_mult * delta_abs
        sl_pct = (atr_sl_move / premium) if premium > 0 else self.min_sl_pct
        sl_pct = min(max(sl_pct, self.min_sl_pct), self.max_sl_pct)
        sl_price = premium * (1 - sl_pct)
    
        atr_tp_move = abs(atr_points) * tp_mult * delta_abs
        tp_price = premium + atr_tp_move
    
        # Optional: cap/adjust by OI-based S/R moves (in underlying points)
        if oi_target_move is not None and oi_target_move > 0:
            oi_tp_price = premium + (float(oi_target_move) * delta_abs)
            tp_price = min(tp_price, oi_tp_price)
    
        if oi_stop_move is not None and oi_stop_move > 0:
            oi_sl_price = premium - (float(oi_stop_move) * delta_abs)
            sl_price = max(sl_price, oi_sl_price)
    
        return round(sl_price, 2), round(tp_price, 2), sl_pct

class DynamicTrailingSL:
    """
    ATR-layered trailing stop for option BUY.

    Goals (based on your CSV behaviour):
    - No 'PROFIT_LOCK' exit until *after* the trade has moved in profit.
    - ATR-based levels (L1/L2/L3) to manage BE, lock, and runner trailing.
    - Optional trailing-target that keeps extending TP once runner mode starts.
    """
    def __init__(
        self,
        entry_price: float,
        atr_points: float,
        *,
        l1_mult: float = 0.8,   # Level-1 target = entry + ATR*0.8
        l2_mult: float = 1.3,   # Level-2 target = entry + ATR*1.3
        l3_mult: float = 2.0,   # Level-3 target = entry + ATR*2.0 (runner)
        lock2_mult: float = 0.3, # After L2, lock = entry + ATR*0.3
        trail_mult: float = 0.7, # After L3, trail gap = ATR*0.7
        min_atr_pct: float = 0.12 # ATR floor vs premium
    ):
        self.entry = float(entry_price)
        # robust ATR for low-premium options
        atr = float(atr_points or 0.0)
        atr_floor = max(self.entry * float(min_atr_pct), 0.05)
        self.atr = max(atr, atr_floor)

        self.max_price = self.entry
        self.sl: Optional[float] = None
        self.tp: Optional[float] = None

        self.l1 = self.entry + self.atr * float(l1_mult)
        self.l2 = self.entry + self.atr * float(l2_mult)
        self.l3 = self.entry + self.atr * float(l3_mult)

        self.lock2 = self.entry + self.atr * float(lock2_mult)
        self.trail_gap = self.atr * float(trail_mult)

        self.runner_mode = False

    def update(self, ltp: float) -> tuple[Optional[float], Optional[float]]:
        """Returns (new_sl, new_tp). Values are None until activated."""
        ltp = float(ltp)

        if ltp > self.max_price:
            self.max_price = ltp

        # Activate only after profit (avoid loss-exit labeled as profit lock)
        if self.max_price >= self.l1:
            # Breakeven after L1
            self.sl = max(self.sl or 0.0, self.entry)

        if self.max_price >= self.l2:
            # Lock partial after L2
            self.sl = max(self.sl or 0.0, self.lock2)

        if self.max_price >= self.l3:
            # Runner trailing after L3
            self.runner_mode = True
            trail_sl = self.max_price - self.trail_gap
            self.sl = max(self.sl or 0.0, trail_sl)

            # Trailing-target (keeps stretching TP in runner mode)
            # If you want a hard TP instead, keep tp at self.l3.
            self.tp = max(self.tp or 0.0, self.max_price + (self.atr * 0.5))
        else:
            # Before runner mode, use layered TP (L2 as default take-profit)
            self.tp = self.l2

        out_sl = round(self.sl, 2) if self.sl is not None else None
        out_tp = round(self.tp, 2) if self.tp is not None else None
...
class OrderManagerV2:
    ...
    def _process_entry(
        ...
        *,
        ...
        try:
            ...
            print(f"🚫 [ENTRY] Skipped due to margin shortfall: {e}")
            return False
        ...
        if ltp <= 0:
            hint = 0.0
            try:
                hint = float((indsP or {}).get("entry_hint") or 0.0)
            except Exception:
                hint = 0.0
            if hint > 0:
                _logger_v2.warning(
                    "ENTRY_LTP_FALLBACK | symbol=%s | broker_ltp=%.4f -> using entry_hint=%.4f",
                    symbol,
                    ltp,
                    hint,
                )
                ...
                _logger_v2.error(
                    "ENTRY_REJECTED | symbol=%s | invalid ltp=%.4f and no entry_hint",
                    symbol,
                    ltp,
                )
                return False
        ...
        tp_override = getattr(self, 'tp_price_override', None)
        sl_override = getattr(self, 'sl_price_override', None)
        try:
            tp_override = float(tp_override) if tp_override is not None else None
        except Exception:
            tp_override = None
        try:
            sl_override = float(sl_override) if sl_override is not None else None
        except Exception:
            sl_override = None
        self.tp_price = tp_override if (tp_override is not None and tp_override > 0) else float(ltp * (1 + tp_pct))
        self.sl_price = sl_override if (sl_override is not None and sl_override > 0) else float(ltp * (1 - sl_pct))
        atr_val = float(indsP.get("atr", 0)) if indsP else 0.0
        atr_points = atr_val
        ...
        self.atr_trail = DynamicTrailingSL(self.entry_price, atr_points)
        # Initial SL from caller (OptionOrderManagerV2.execute_signal)
        self.atr_trail.sl = float(self.sl_price)
    ...
    def execute_unified_strategy(self, ltp: float) -> Tuple[bool, str]:
        ...
        # 1) STOP LOSS FIRST
        # Price-based ATR SL (preferred)
        if self.sl_price is not None and float(ltp) <= float(self.sl_price):
            return True, f"STOP_LOSS_ATR_PRICE(ltp={float(ltp):.2f} <= sl={float(self.sl_price):.2f})"
        ...
        # 3.5) ATR-LAYERED PROFIT LOCK + TRAILING (CSV-tuned)
        if self.atr_trail:
            new_sl, new_tp = self.atr_trail.update(ltp)
            # Tighten SL/TP dynamically; do NOT exit here. Exit is handled by SL/TP checks.
    ...
    def _process_exit(self, reason: str, ltp: Optional[float] = None) -> Dict[str, Any]:
        ...
        self.tp_price = None
        self.atr_trail = None
        self.point_trail = None
        for _k in ("sl_price_override", "tp_price_override"):
            if hasattr(self, _k):
                delattr(self, _k)
...
class OptionEngineV3:
    ...
    def _momentum(self, cfg: UnderlyingConfigV2, symbol: str) -> Tuple[str, Dict[str, Any]]:
        ...
        info = {"ema_fast": round(last_f, 4), "ema_slow": round(last_s, 4), "price": float(close.iloc[-1]), "prev_price": float(close.iloc[-2]), "price_chg": float(close.iloc[-1] - close.iloc[-2])}
        if last_f > last_s:
            return "BULLISH", info
        if last_f < last_s:
            return "BEARISH", info
...
# ---------------------------------------------------------------------
# OI Build-up Classification + Strike-wise Scoring
# ---------------------------------------------------------------------
    def _oi_buildup_label(self, price_chg: float, oi_chg: float) -> str:
        """Classify build-up using underlying price change + OI change.
    
        Using standard futures/OI interpretation:
        ...
        """
        try:
            pc = float(price_chg or 0.0)
            oc = float(oi_chg or 0.0)
        except Exception:
            return 'NEUTRAL'
        if abs(pc) < 1e-9 or abs(oc) < 1e-9:
            return 'NEUTRAL'
        if pc > 0 and oc > 0:
            return 'LONG_BUILDUP'
        if pc > 0 and oc < 0:
    ...
    def _oi_regime_score_from_chain(self, rows: list, underlying_ltp: float, price_chg: float) -> dict:
        """Compute market bias score from strike-wise OI changes.
    
        - Finds OI-based Support/Resistance from max PE/CE OI.
        - Evaluates CE/PE OI change regimes at key strikes (ATM, Support, Resistance, Support2, Resistance2).
        - Produces a single signed score:
            +ve => bullish bias, -ve => bearish bias.
        """
        if not rows:
            return {'score': 0.0, 'bias': 'NEUTRAL', 'details': {}}
        ...
        for r in rows:
            try:
                ...
                if opt_type not in ('CE','PE'):
                    continue
                by_strike.setdefault(strike, {})[opt_type] = r
            except Exception:
                continue
        ...
        # Support/Resistance by max OI
        def _row_oi_safe(rr):
            try:
                return float(self._row_oi(rr) or 0.0)
            except Exception:
                return 0.0
        ...
        for s, d in by_strike.items():
            if 'PE' in d:
                pe_sorted.append((s, _row_oi_safe(d['PE'])))
            if 'CE' in d:
                ce_sorted.append((s, _row_oi_safe(d['CE'])))
        pe_sorted.sort(key=lambda x: x[1], reverse=True)
        ce_sorted.sort(key=lambda x: x[1], reverse=True)
    
        support = pe_sorted[0][0] if pe_sorted else None
        support2 = pe_sorted[1][0] if len(pe_sorted)>1 else None
        resistance = ce_sorted[0][0] if ce_sorted else None
        resistance2 = ce_sorted[1][0] if len(ce_sorted)>1 else None
    
        # ATM strike (nearest)
        strikes_sorted = sorted(by_strike.keys())
        atm = min(strikes_sorted, key=lambda s: abs(s-underlying_ltp)) if underlying_ltp>0 else strikes_sorted[len(strikes_sorted)//2]
    
        key_strikes=[]
        for k in (atm, support, resistance, support2, resistance2):
            if k is None:
                continue
            if k not in key_strikes:
                key_strikes.append(k)
    
        # Scoring
        # CE regimes: LONG_BUILDUP / SHORT_COVERING => bullish (+)
        # PE regimes: LONG_BUILDUP / SHORT_COVERING => bearish (-)
        import math
        norm = 50000.0
        details={
            'atm': atm,
        ...
        for ks in key_strikes:
            d = by_strike.get(ks, {})
            w = 1.0
            if ks in (support, resistance):
                w = 1.5
            if ks in (support2, resistance2):
                w = max(w, 1.2)
    
            # CE
            if 'CE' in d:
                oi_chg = float(self._row_oi_change(d['CE']) or 0.0)
                lab = self._oi_buildup_label(price_chg, oi_chg)
                mag = math.log1p(abs(oi_chg)) / math.log1p(norm) if norm>0 else 1.0
                mag = max(0.0, min(1.0, mag))
                if lab == 'LONG_BUILDUP':
                    score += (1.0 * w * mag)  # Strongly Bullish
                elif lab == 'SHORT_COVERING':
                    score += (0.5 * w * mag)  # Bullish
                elif lab == 'SHORT_BUILDUP':
                    score -= (1.0 * w * mag)  # Strongly Bearish
                elif lab == 'LONG_UNWINDING':
                    score -= (0.5 * w * mag)  # Bearish
                details['strikes'].setdefault(str(ks), {})['CE']={'oi_chg': oi_chg, 'label': lab, 'w': w, 'mag': mag}
    
            # PE
            if 'PE' in d:
                oi_chg = float(self._row_oi_change(d['PE']) or 0.0)
                lab = self._oi_buildup_label(-price_chg, oi_chg)
                mag = math.log1p(abs(oi_chg)) / math.log1p(norm) if norm>0 else 1.0
                mag = max(0.0, min(1.0, mag))
                if lab == 'SHORT_BUILDUP':
                    score += (1.0 * w * mag)  # Puts sold (Price DOWN/OI UP) => Strongly Bullish
                elif lab == 'LONG_UNWINDING':
                    score += (0.5 * w * mag)  # Puts closed (Price DOWN/OI DOWN) => Bullish
                elif lab == 'LONG_BUILDUP':
                    score -= (1.0 * w * mag)  # Puts bought (Price UP/OI UP) => Strongly Bearish
                elif lab == 'SHORT_COVERING':
                    score -= (0.5 * w * mag)  # Puts covered (Price UP/OI DOWN) => Bearish
                details['strikes'].setdefault(str(ks), {})['PE']={'oi_chg': oi_chg, 'label': lab, 'w': w, 'mag': mag}
        ...
        if score >= 3.0:
            ...
            bias='BEARISH'
        return {'score': round(score, 3), 'bias': bias, 'details': details}
    ...
    def _row_option_type(self, r: Dict[str, Any]) -> Optional[str]:
        ...
        # Fallback: infer from trading symbol text when type fields are missing/blank.
        sym = str(
            r.get("symbol")
            or r.get("name")
        ...
        if sym.endswith("CE") or "-CE" in sym or " CE" in sym:
            return "CE"
        if sym.endswith("PE") or "-PE" in sym or " PE" in sym:
            return "PE"

        desc = str(r.get("description") or r.get("desc") or "").strip().upper()
        if "CALL" in desc:
            return "CE"
        if "PUT" in desc:
            return "PE"
    ...
    def _row_numeric(self, r: Dict[str, Any], keys: Tuple[str, ...], default: float = 0.0) -> float:
        """Extract first available numeric field from row."""
        for k in keys:
            try:
    ...
    def _row_oi(self, r: Dict[str, Any]) -> float:
        return self._row_numeric(r, ("oi", "open_interest", "openInterest"), default=0.0)

    def _row_oi_change(self, r: Dict[str, Any]) -> float:
        return self._row_numeric(
            r,
            (
                "oich",
                "oi_chg",
    ...
    def _time_to_expiry_years(self, expiry_val: Any, symbol_hint: str = "") -> float:
        ...
        - Symbol-name extraction (e.g., MCX:NATGASMINI26MAR290PE → 24-Mar-2026)

        Falls back to 2 days if parsing fails (with a warning).
        """
        import re as _re_tte
        try:
            now = _dt.datetime.now(_IST_V3)
            # epoch
            if isinstance(expiry_val, (int, float)) and float(expiry_val) > 1e6:
                ts = float(expiry_val)
                if ts > 1e12:
                    ts = ts / 1000.0
        ...
        # ── NEW: Extract expiry from symbol name ──
        # e.g., MCX:NATGASMINI26MAR290PE → "26MAR" → 24-Mar-2026 (MCX expiry = 24th of month)
        # e.g., BSE:SENSEX26228CE → "2622" → 22-Feb-2026
        _sym = str(symbol_hint or expiry_val or "").upper()
        _MONTHS = {'JAN':1,'FEB':2,'MAR':3,'APR':4,'MAY':5,'JUN':6,
                   'JUL':7,'AUG':8,'SEP':9,'OCT':10,'NOV':11,'DEC':12}
        try:
            # Pattern: YYMMM (e.g., 26MAR, 26FEB) — MCX style
            m = _re_tte.search(r'(\d{2})(JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)', _sym)
            if m:
                yy = int(m.group(1))
                mm = _MONTHS[m.group(2)]
                # MCX options expire on the 24th of the expiry month (or prior business day)
                # Using 24th as the standard expiry day
                exp_year = 2000 + yy
                exp_day = 24  # MCX standard expiry day
                try:
                    exp = _dt.datetime(exp_year, mm, exp_day, 15, 30, tzinfo=_IST_V3)  # 3:30 PM close
                except ValueError:
                    # If 24th is invalid (shouldn't happen), use last day of month
                    import calendar
                    exp_day = calendar.monthrange(exp_year, mm)[1]
                    exp = _dt.datetime(exp_year, mm, exp_day, 15, 30, tzinfo=_IST_V3)
                now = _dt.datetime.now(_IST_V3)
                T = max((exp - now).total_seconds(), 0.0) / (365.0 * 24 * 3600)
                # print(f"[EXPIRY-PARSE] Symbol={_sym} → Expiry={exp.strftime('%Y-%m-%d')} | T={T:.6f}yr ({T*365:.1f}d)")
                return T
        except Exception as e:
            print(f"[EXPIRY-PARSE] Failed to extract from symbol '{_sym}': {e}")

        # fallback: 2 trading days — WARN this is unreliable
        print(f"[WARN] [EXPIRY] Could not parse expiry for '{expiry_val}' (symbol='{symbol_hint}'). Using FALLBACK T=2 days. Greeks may be UNRELIABLE.")
        return 2.0 / 365.0

    def _calculate_greeks_for_row(self, r: Dict[str, Any], cfg: UnderlyingConfigV2, underlying_ltp: float) -> GreeksV2:
        """Centralized Greek calculation with Black-76 and IV normalization.
        
        Since Fyers API does NOT return Greeks in the option chain response,
        we always compute them via Black-76 using:
          F = underlying futures LTP (must match option's expiry series)
          K = strike price
          T = time to expiry in years
          σ = implied volatility (solved via bisection from market premium)
          r = risk-free rate (0.0 for MCX commodities)
        """
        ...
        # --- Underlying/Forward price from row (Self-correction) ---
        # Fyers chain may return the forward/underlying price for that expiry.
        u_p = float(r.get("underlying_price") or r.get("spot_price") or r.get("strike_price_underlying") or 0.0)
        if u_p > 0:
            underlying_ltp = u_p

        # --- Try to get real-time futures LTP for better accuracy ---
        # For MCX options, we should use the near-month futures price as F
        _F_used = underlying_ltp  # Track what forward price we actually use

        # --- Implied Volatility Normalization ---
        # Fyers API usually returns IV as percentage (e.g. 18.5 for 18.5%).
        # Black-76 math requires decimal (0.185).
        if iv_val > 1.5:  
             iv_calc = iv_val / 100.0
        else:
             iv_calc = iv_val

        # --- Forced Recalculation Condition ---
        # Since Fyers API doesn't return Greeks, recalculate always when data is present.
        recalculate = (
            abs(delta_val) < 1e-6 or 
            abs(gamma_val) < 1e-7 or 
            iv_calc <= 0
        )

        if recalculate and _F_used > 0 and K > 0 and ltp > 0:
            try:
                # Pass symbol to expiry parser for symbol-name extraction
                T = float(self._time_to_expiry_years(
                    r.get("expiry") or r.get("expDate") or "",
                    symbol_hint=symbol
                ))
                # Default risk-free rate logic: commodities (MCX) often use 0 in terminal displays.
                is_mcx = symbol.startswith("MCX:")
                rfr_default = 0.0 if is_mcx else 0.07
                rfr = float(getattr(cfg, "risk_free_rate", rfr_default) or rfr_default)
                
                opt_kind = 'C' if opt_type == 'CE' else 'P'
                sigma = iv_calc if (iv_calc > 0 and not recalculate) else float(
                    Black76Greeks.implied_vol(_F_used, K, rfr, T, opt_kind, ltp)
                )
                
                if sigma > 0:
                    gg = Black76Greeks.greeks(_F_used, K, rfr, sigma, T, opt_kind)
                    ...
                    # --- Model Price Sanity Check ---
                    model_price = Black76Greeks.price(_F_used, K, rfr, sigma, T, opt_kind)
                    price_err_pct = abs(model_price - ltp) / ltp * 100 if ltp > 0 else 0.0

                    # Diagnostic log: commented out to reduce log spam
                    # print(
                    #     f"[B76-DIAG] {symbol} | F={_F_used:.2f} K={K:.1f} T={T*365:.1f}d "
                    #     f"σ={sigma*100:.1f}% r={rfr:.3f} | type={opt_kind} "
                    #     f"| Δ={delta_val:+.4f} Γ={gamma_val:.5f} Θ={theta_val:.4f}/d "
                    #     f"| IV={sigma*100:.2f}% | mkt_ltp={ltp:.2f} model={model_price:.2f} "
                    #     f"err={price_err_pct:.1f}%"
                    # )
                    if price_err_pct > 10.0:
                        print(
                            f"[WARN] [B76-DIAG] Model price deviation {price_err_pct:.1f}% for {symbol}. "
                            f"Inputs may be misaligned (F/T/K). Greeks may be UNRELIABLE. "
                            f"(F={_F_used:.2f} K={K:.1f} ltp={ltp:.2f} model={model_price:.2f})"
                        )
            except Exception as e:
                print(f"[ERROR] [B76] Greeks calc failed for {symbol}: {e}")
    ...
    def _pick_atm_from_chain(
        ...
        *,
        ...
        for r in rows:
            ...
            sym_u = sym_text.upper()
            sym_hint = "CE" if (sym_u.endswith("CE") or "-CE" in sym_u or " CE" in sym_u) else (
                "PE" if (sym_u.endswith("PE") or "-PE" in sym_u or " PE" in sym_u) else ""
            )
            if opt_type != desired_type:
                # Relaxed match by symbol suffix if payload type field is unreliable.
                if sym_hint != desired_type:
                    continue
            strike = float(r.get("strike_price") or r.get("strike") or 0.0)
            if strike <= 0:
                continue
        ...
        best_symbol = str(
            best_row.get("symbol")
            or best_row.get("name")
            or best_row.get("tradingsymbol")
            or best_row.get("symbolName")
            or ""
        ).strip()
        if not best_symbol:
            return None
        ...
        meta = {
            ...
            "bid": bid,
            "ask": ask,
            "oi": self._row_oi(best_row),
            "oi_change": self._row_oi_change(best_row),
    ...
    def analyze(
        ...
        *,
        ...
        # --- OI Build-up Regime (strike-wise scoring) ---
        price_chg = float((mom_info or {}).get('price_chg') or 0.0)
        # Underlying LTP for regime computation (best-effort)
        _ul = float(underlying_ltp or 0.0)
        if _ul <= 0:
            _ul = self._get_underlying_ltp(resolved.analysis_symbol)
        oi_regime = self._oi_regime_score_from_chain(rows, _ul, price_chg)
        # Bias entries: reject only when regime is strongly opposite
        rej_thr = float(getattr(cfg, 'oi_regime_reject_score', 2.0) or 2.0)
        if desired_type == 'CE' and oi_regime.get('score', 0.0) <= -rej_thr:
            print(f"[OI-REGIME] REJECT CE | score={oi_regime.get('score')} bias={oi_regime.get('bias')} | price_chg={price_chg:.2f}")
            return None
        if desired_type == 'PE' and oi_regime.get('score', 0.0) >= rej_thr:
            print(f"[OI-REGIME] REJECT PE | score={oi_regime.get('score')} bias={oi_regime.get('bias')} | price_chg={price_chg:.2f}")
            return None

        # Add regime + OI-SR summary into extra_meta so it flows into signal.meta
        extra_meta = dict(extra_meta or {})
        oi_details = oi_regime.get('details') or {}
        ...
        support = _as_float(oi_details.get('support'))
        support2 = _as_float(oi_details.get('support2'))
        resistance = _as_float(oi_details.get('resistance'))
        resistance2 = _as_float(oi_details.get('resistance2'))
        spot_ref = _as_float(_ul) or 0.0

        buf_pts = float(getattr(cfg, 'oi_sr_buffer', 25.0) or 25.0)
        tgt_level = None
        stp_level = None
        if spot_ref > 0 and support is not None and resistance is not None:
            if desired_type == 'CE':
                tgt_level = max(spot_ref, resistance - buf_pts)
                stp_level = min(spot_ref, support - buf_pts)
            else:
                tgt_level = min(spot_ref, support + buf_pts)
                stp_level = max(spot_ref, resistance + buf_pts)

        tgt_move = abs(tgt_level - spot_ref) if (tgt_level is not None and spot_ref > 0) else None
        stp_move = abs(spot_ref - stp_level) if (stp_level is not None and spot_ref > 0) else None
        rr_est = (tgt_move / stp_move) if (tgt_move is not None and stp_move not in (None, 0)) else None
        ...
        oi_sr = {
            ...
            'resistance': resistance,
            'resistance2': resistance2,
            'target_level': float(tgt_level) if tgt_level is not None else None,
            'stop_level': float(stp_level) if stp_level is not None else None,
            'target_move': float(tgt_move) if tgt_move is not None else None,
            'stop_move': float(stp_move) if stp_move is not None else None,
            'rr_est': float(rr_est) if rr_est is not None else None,
            'regime_score': float(oi_regime.get('score', 0.0) or 0.0),
            'regime_bias': oi_regime.get('bias', 'NEUTRAL'),
        }

        extra_meta['oi_regime_score'] = oi_regime.get('score', 0.0)
        extra_meta['oi_regime_bias'] = oi_regime.get('bias', 'NEUTRAL')
        extra_meta['oi_regime_details'] = oi_details
        extra_meta['oi_sr'] = oi_sr
        ...
        # ── OI Regime Meaning Map ──
        _oi_meaning = {
            'LONG_BUILDUP':   'Price\u2191 + OI\u2191 = Fresh Buying (Bullish)',
            'SHORT_COVERING': 'Price\u2191 + OI\u2193 = Shorts Exiting (Bullish)',
            'SHORT_BUILDUP':  'Price\u2193 + OI\u2191 = Fresh Selling (Bearish)',
        ...
        _oi_score = oi_regime.get('score', 0.0)
        _oi_bias  = oi_regime.get('bias', 'NEUTRAL')

        print(f"")
        print(f"{'='*70}")
        print(f"  [DATA] OI REGIME ANALYSIS \u2014 {cfg.key} ({desired_type})")
        print(f"{'\u2500'*70}")
        print(f"  Underlying Price Change : {price_chg:+.2f}")
        print(f"  OI Regime Score         : {_oi_score:+.3f}")
        print(f"  Market Bias             : {_bias_emoji.get(_oi_bias, '')} {_oi_bias}")
        print(f"{'\u2500'*70}")
        print(f"  [POS] SUPPORT / RESISTANCE (by max OI)")
        print(f"     Spot   = {_fmt(spot_ref)}")
        print(f"     S1     = {_fmt(support)}   |  S2 = {_fmt(support2)}")
        print(f"     R1     = {_fmt(resistance)}   |  R2 = {_fmt(resistance2)}")
        print(f"     Target = {_fmt(tgt_level)}   |  Stop = {_fmt(stp_level)}")
        if tgt_move is not None and stp_move is not None:
            print(f"     Move   = Target {_fmt(tgt_move)} pts / Stop {_fmt(stp_move)} pts  |  R:R = {_fmt(rr_est)}")
        print(f"{'\u2500'*70}")
        print(f"  [DATA] STRIKE-WISE OI BUILD-UP LABELS")
        print(f"  {'Strike':<12} {'CE Label':<20} {'CE dOI':>10}  {'PE Label':<20} {'PE dOI':>10}")
        print(f"  {'\u2500'*12} {'\u2500'*20} {'\u2500'*10}  {'\u2500'*20} {'\u2500'*10}")

        _strike_stats = oi_details.get('strikes') or {}
        for _k in (oi_details.get('key_strikes') or [])[:5]:
            _d = _strike_stats.get(str(_k), {})
            _ce = _d.get('CE') or {}
            _pe = _d.get('PE') or {}
            _ce_lbl = _ce.get('label', 'NA')
            _pe_lbl = _pe.get('label', 'NA')
            _ce_chg = _as_float(_ce.get('oi_chg'))
            _pe_chg = _as_float(_pe.get('oi_chg'))
            _ce_e = _oi_emoji.get(_ce_lbl, '[ ]')
            _pe_e = _oi_emoji.get(_pe_lbl, '[ ]')
            _ce_oi_str = f"{int(_ce_chg):+,}" if _ce_chg is not None else "NA"
            _pe_oi_str = f"{int(_pe_chg):+,}" if _pe_chg is not None else "NA"
            print(f"  {_fmt(_k):<12} {_ce_e} {_ce_lbl:<18} {_ce_oi_str:>10}  {_pe_e} {_pe_lbl:<18} {_pe_oi_str:>10}")

        print(f"{'\u2500'*70}")
        print(f"  Legend: [+] LONG_BUILDUP (Fresh Buy) | [^] SHORT_COVERING (Shorts Exit)")
        print(f"          [-] SHORT_BUILDUP (Fresh Sell) | [v] LONG_UNWINDING (Longs Exit)")
        print(f"{'='*70}")
        print(f"")
        ...
        for r in rows:
            ...
            ltp = float(r.get("ltp") or r.get("lp") or 0.0)
            oi_val = self._row_oi(r)
            oi_chg_val = self._row_oi_change(r)
        ...
        if not candidates:
            fallback_enabled = bool(force_atm or getattr(cfg, "fallback_on_no_candidates", True) or getattr(cfg, "allow_no_greeks_fallback", True))
            if fallback_enabled:
                print(f"[WARN] [ENGINE] No candidates passed strict filters for {cfg.key} {desired_type}")
            else:
            ...
            do_fallback = False
            if no_data:
                allow_no_greeks_fallback = bool(getattr(cfg, "allow_no_greeks_fallback", True))
                if allow_no_greeks_fallback:
                    print(
                        f"[WARN] [ENGINE] Greeks missing/zero (ADX={adx_val:.1f}). "
                        "Triggering ATM fallback..."
                    )
                    do_fallback = True
                    self.fallbacks_today += 1
                else:
                    print("[SKIP] [ENGINE] Greeks missing/zero and fallback disabled by config.")
            else:
                # Candidates exist but strict filters removed all.
                # Use ATM fallback so the engine does not deadlock on overly-tight ranges.
                strict_filter_block = bool(getattr(cfg, "fallback_on_no_candidates", True))
                if strict_filter_block:
                    print("[WARN] [ENGINE] Strict filters removed all candidates. Triggering ATM fallback...")

            if force_atm or do_fallback or strict_filter_block:
                fb_reason = "FORCE_ATM" if force_atm else ("GREEKS_MISSING" if do_fallback else "NO_CANDIDATES_FILTER")
...
class TradingConfigZeroToHero:
    ...
    ENABLE_TIME_FILTERS = False            # NEW: disable time-based entry/exit constraints
...
class ExpiryDayFilters:
    ...
    def check_time_filter(self, current_time: dt_time) -> Tuple[bool, str]:
        """Time filters DISABLED (live OHLC/LTP).

        User requested: no time-based entry blocking and no time-based exits.
        Strategy decisions are purely indicator/structure-driven.
        """
...
class OptionOrderManagerV2:
    ...
    def execute_signal(self, signal: OptionBuySignalV2, cfg: UnderlyingConfigV2) -> bool:
        ...
        # OI-based S/R adjustment inputs (underlying points)
        oi_sr = None
        try:
            oi_sr = (signal.meta or {}).get("oi_sr") if hasattr(signal, "meta") else None
        except Exception:
            oi_sr = None
        if not oi_sr:
            oi_sr = getattr(signal, "oi_sr", None)

        _oi_target_move = None
        _oi_stop_move = None
        try:
            spot_ref = float((oi_sr or {}).get("spot") or getattr(signal, "spot", 0.0) or 0.0)
            support = (oi_sr or {}).get("support")
            resistance = (oi_sr or {}).get("resistance")
            buf = float(getattr(cfg, "oi_sr_buffer", 25.0))  # underlying points buffer
            opt_type = str(getattr(signal, "option_type", "") or "").upper()

            if spot_ref > 0 and support and resistance:
                support = float(support)
                resistance = float(resistance)

                if opt_type == "CE":
                    # target until resistance (minus buffer)
                    tgt_lvl = max(spot_ref, resistance - buf)
                    stp_lvl = min(spot_ref, support - buf)
                    _oi_target_move = max(0.0, tgt_lvl - spot_ref)
                    _oi_stop_move = max(0.0, spot_ref - stp_lvl)
                elif opt_type == "PE":
                    # target until support (plus buffer)
                    tgt_lvl = min(spot_ref, support + buf)
                    stp_lvl = max(spot_ref, resistance + buf)
                    _oi_target_move = max(0.0, spot_ref - tgt_lvl)
                    _oi_stop_move = max(0.0, stp_lvl - spot_ref)
        except Exception:
            _oi_target_move = None
            _oi_stop_move = None
        ...
        sl_price_target, tp_price_target, atr_sl_pct = risk_mgr.compute_sl_tp(
            ...
            atr_points=atr_val,
            delta_abs=abs(float(getattr(signal.greeks, 'delta', 0.0) or 0.0)) if hasattr(signal, 'greeks') else 0.5,
            oi_target_move=_oi_target_move,
            oi_stop_move=_oi_stop_move,
            regime_score=float((oi_sr or {}).get('regime_score') or (getattr(signal,'meta',{}) or {}).get('oi_regime_score') or 0.0),
            opt_type=str(getattr(signal, 'option_type', '') or ''),
        )
        ...
        indsP = {
            ...
            "entry_hint": float(signal.entry_price or 0.0)
        }
...
""")
        ...
        if atr_val > 0 and sl_price_target > 0 and tp_price_target > 0:
            self.om.sl_price_override = float(sl_price_target)
            self.om.tp_price_override = float(tp_price_target)
        else:
            for _k in ("sl_price_override", "tp_price_override"):
                if hasattr(self.om, _k):
                    delattr(self.om, _k)
...
class TradingBotV2:
    ...
    def get_weighted_bias(self, symbol, inds):
        ...
        st14_dir = _to_sign(_get_val("st14_dir", 0))
        st21_dir = _to_sign(_get_val("st21_dir", 0))
        st14_sig = _to_sign(_get_val("st_14_2_signal", 0))
        st21_sig = _to_sign(_get_val("st_21_1_signal", 0))
        st14 = st14_dir if st14_dir != 0 else st14_sig
        st21 = st21_dir if st21_dir != 0 else st21_sig
    ...
    def option_buying_vote(self, fut_inds, opt_inds, option_type, debug=True):
        """
        ...
        - Synchronized with MERGED.py institutional logic
        """        # ------------------------------------------------------------------
        # 0) Trend direction is decided ONLY by Underlying SuperTrend:
        #    - Underlying ST14/2 + ST21/1 GREEN => BUY CE
        #    - Underlying ST14/2 + ST21/1 RED   => BUY PE
        # ------------------------------------------------------------------
        def _to_sign(v):
            try:
                fv = float(v)
        ...
        # Normalize ST from both possible key-sets and prefer *_dir when available.
        fut_st14_dir = _to_sign(fut_inds.get("st14_dir"))
        fut_st21_dir = _to_sign(fut_inds.get("st21_dir"))
        fut_st14_sig = _to_sign(fut_inds.get("st_14_2_signal"))
        fut_st21_sig = _to_sign(fut_inds.get("st_21_1_signal"))

        fut_st14 = fut_st14_dir if fut_st14_dir != 0 else fut_st14_sig
        fut_st21 = fut_st21_dir if fut_st21_dir != 0 else fut_st21_sig

        if debug and (
            (fut_st14_dir != 0 and fut_st14_sig != 0 and fut_st14_dir != fut_st14_sig) or
            (fut_st21_dir != 0 and fut_st21_sig != 0 and fut_st21_dir != fut_st21_sig)
        ):
            print(
                f"[VOTE] ST source mismatch | dir=({fut_st14_dir},{fut_st21_dir}) "
                f"sig=({fut_st14_sig},{fut_st21_sig}) -> using ({fut_st14},{fut_st21})"
            )

        derived_type = None
        need_dir = 0   # +1 => bullish (CE), -1 => bearish (PE)

        if fut_st14 == 1 and fut_st21 == 1:
            derived_type = "CE"
            need_dir = 1
        elif fut_st14 == -1 and fut_st21 == -1:
            derived_type = "PE"
            need_dir = -1
        else:
            # No clear trend => do NOT trade (avoid chop)
            if debug:
                print(f"[VOTE] Underlying ST not aligned (ST14={fut_st14}, ST21={fut_st21}) => NO TRADE")
            return 0

        # override whatever caller passed (to avoid mismatch bugs)
        option_type = derived_type
        self._last_vote_option_type = option_type

        # ------------------------------------------------------------------
        # 0.5) HARD GATE: OI data must be present and supportive
        # ------------------------------------------------------------------
        oi_data = opt_inds.get("oi", None)
        oi_chg = opt_inds.get("oi_change", None)
        try:
            oi_data = float(oi_data) if oi_data not in (None, "") else None
        except Exception:
            oi_data = None
        try:
            oi_chg = float(oi_chg) if oi_chg not in (None, "") else None
        except Exception:
            oi_chg = None

        # Semi-hard OI gate:
        # 1) OI data must be present
        # 2) Block only when OI change is strongly opposite (symbol-wise threshold)
        if oi_data is None or oi_data <= 0:
            if debug:
                print(f"[VOTE] ❌ OI data unavailable/invalid (OI={oi_data}) => NO TRADE")
            return 0
        if oi_chg is None:
            if debug:
                print(f"[VOTE] ❌ OI change unavailable (Chg={oi_chg}) => NO TRADE")
            return 0

        sym_tag = str(fut_inds.get("symbol") or fut_inds.get("underlying") or "").upper()
        if "SENSEX" in sym_tag:
            oi_chg_threshold = max(1500.0, 0.04 * float(oi_data))
        elif "NATGASMINI" in sym_tag or "NATGAS" in sym_tag:
            oi_chg_threshold = max(400.0, 0.03 * float(oi_data))
        else:
            oi_chg_threshold = max(500.0, 0.03 * float(oi_data))

        if option_type == "CE" and oi_chg >= oi_chg_threshold:
            if debug:
                print(f"[VOTE] ❌ CE OI strongly opposite (chg={oi_chg:.0f} >= {oi_chg_threshold:.0f}) => NO TRADE")
            return 0
        if option_type == "PE" and oi_chg <= -oi_chg_threshold:
            if debug:
                print(f"[VOTE] ❌ PE OI strongly opposite (chg={oi_chg:.0f} <= -{oi_chg_threshold:.0f}) => NO TRADE")
            return 0

        oi_gate_ok = True
        if debug:
            print(f"[VOTE] ✅ OI Check PASSED (OI={oi_data}, Chg={oi_chg}, Thr={oi_chg_threshold:.0f})")

        # ------------------------------------------------------------------
        # 1) HARD GATE: Selected option itself must be trending UP (GREEN ST)
        #    IMPORTANT: For PE also, option price must be in GREEN because
        #    you are BUYING the option premium.
        # ------------------------------------------------------------------
        opt_st14_dir = _to_sign(opt_inds.get("st14_dir"))
        opt_st21_dir = _to_sign(opt_inds.get("st21_dir"))
        opt_st14_sig = _to_sign(opt_inds.get("st_14_2_signal"))
        opt_st21_sig = _to_sign(opt_inds.get("st_21_1_signal"))
        opt_st14 = opt_st14_dir if opt_st14_dir != 0 else opt_st14_sig
        opt_st21 = opt_st21_dir if opt_st21_dir != 0 else opt_st21_sig
        if not (opt_st14 == 1 and opt_st21 == 1):
            if debug:
                print(f"[VOTE] Option ST gate FAILED (opt ST14={opt_st14}, opt ST21={opt_st21}) => NO TRADE")
            return 0
        ...
        print(f"  Future/Index Trend : ST14={fut_st14}, ST21={fut_st21}")
        ...
        # ─────────────────────────────────────────
        # DYNAMIC THRESHOLD (RELAXED INTRADAY)
        # - Base PASS=3 (instead of 4)
        # - Early day + compression => require 4
        # - Late day (>=14:30 IST) => allow 2 (avoid missing late trends)
        # ─────────────────────────────────────────
        now_ist = dt.datetime.now(self.IST) if hasattr(self, "IST") else dt.datetime.now()
        # Sanitize ATR ratio (avoid 0.00 edge case)
        if atr_ratio_value is None or atr_ratio_value <= 0.05:
            try:
                atr_base = fut_inds.get("atr_ma50", 0) or atr_fut
                atr_ratio_value = (float(atr_fut) / float(atr_base)) if atr_fut and atr_base else 1.0
            except Exception:
                atr_ratio_value = 1.0

        threshold = 3
        if now_ist.time() < dt_time(10, 0) and atr_ratio_value < 0.85:
            threshold = 4
        if now_ist.time() >= dt_time(14, 30):
            threshold = 2

        # FIX #4: Fast entry mode (high IV/VIX) allows 2-vote entry even earlier
        vix_now = (
            fut_inds.get("vix_now")
            or fut_inds.get("vix")
            or opt_inds.get("vix_now")
            or opt_inds.get("vix")
            or 20.0
        )
        iv_now = (
            opt_inds.get("iv")
            or opt_inds.get("iv_now")
            or fut_inds.get("iv")
            or fut_inds.get("iv_now")
        )
        try:
        ...
        if fast_entry_mode(vix_now=vix_now, iv_now=iv_now):
            threshold = min(threshold, 2)
            print(f"  ✅ FAST ENTRY MODE ON (VIX={vix_now:.1f}, IV={iv_now if iv_now is not None else 'NA'}) → PASS >= {threshold}")

        # Expose to caller
        self._last_vote_threshold = threshold
        print(f"  [VOTE] Dynamic PASS threshold: {threshold} (ATR_R={atr_ratio_value:.2f}, Time={now_ist.strftime('%H:%M')})")
        ...
        if option_type == "CE":
            if fut_st14 == 1 and fut_st21 == 1:
                ...
                print(f"  ❌ FUT Trend not bullish (st14={fut_st14}, st21={fut_st21})")
            ...
            if fut_st14 == -1 and fut_st21 == -1:
                ...
                print(f"  ❌ FUT Trend not bearish (st14={fut_st14}, st21={fut_st21})")
        ...
        ema_tol = 0.002  # 0.2% tolerance to avoid false negatives during pullback/retest candles
        
        if option_type == "CE":
            # CE (Call) requires: close > ema_20 > ema_200 (bullish alignment)
            if close_fut and ema_20 and ema_200:
                strict_bull = (close_fut > ema_5 > ema_20)
                tolerant_bull = (close_fut > ema_20 and ema_5 >= (ema_20 * (1.0 - ema_tol)))
                if strict_bull:
                    score += 1
                    print(f"  ✅ FUT EMA BULLISH (Close {close_fut:.1f} > EMA5 {ema_5:.1f} > EMA20 {ema_20:.1f}): +1")
                elif tolerant_bull:
                    score += 1
                    print(f"  ✅ FUT EMA BULLISH (retest tolerance, Close {close_fut:.1f} > EMA20 {ema_20:.1f}, EMA5~EMA20): +1")
                else:
            ...
            elif atr_ratio_value >= 0.95:
                score += 1
                print(f"  ✅ FUT ATR OK (neutral) (ATR Ratio {atr_ratio_value:.2f} >= 0.95): +1")
            else:
            ...
            if close_fut and ema_20 and ema_200:
                strict_bear = (close_fut < ema_5 < ema_20)
                tolerant_bear = (close_fut < ema_20 and ema_5 <= (ema_20 * (1.0 + ema_tol)))
                if strict_bear:
                    score += 1
                    print(f"  ✅ FUT EMA BEARISH (Close {close_fut:.1f} < EMA5 {ema_5:.1f} < EMA20 {ema_20:.1f}): +1")
                elif tolerant_bear:
                    score += 1
                    print(f"  ✅ FUT EMA BEARISH (retest tolerance, Close {close_fut:.1f} < EMA20 {ema_20:.1f}, EMA5~EMA20): +1")
                else:
        ...
        # ─────────────────────────────────────────
        # 0.65) Momentum confirmation (FIX #7)
        # ─────────────────────────────────────────
        mom_bias = "BULLISH" if option_type == "CE" else "BEARISH"
        mom_inds = fut_inds if isinstance(fut_inds, dict) else {}
        if isinstance(opt_inds, dict) and not (mom_inds.get("mom_z") or mom_inds.get("efi_z")):
            mom_inds = opt_inds
        mom_ok, mom_strength = momentum_ok(mom_inds, mom_bias)
        if mom_ok:
            score += 1
            print(f"  ✅ MOM+EFI aligned (strength={mom_strength:.2f}): +1")
        else:
            print(f"  ⚠ MOM+EFI weak/misaligned (strength={mom_strength:.2f}): +0")
# 0.7) OI supportive context
        # ─────────────────────────────────────────
        if oi_gate_ok:
            score += 1
            print("  ✅ OI supportive: +1")
        else:
            print("  ⚠ OI weak/unavailable: +0")

        # ─────────────────────────────────────────
        # FINAL DECISION
        # ─────────────────────────────────────────
        print(f"\n[VOTE RESULT] FINAL SCORE: {score}/5 (PASS >= {threshold})")
        print(f"[VOTE RESULT] {'✅ BUY OPTION' if score >= threshold else '❌ NO TRADE'}")
        print(f"{'='*70}\n")
    ...
    def _get_voting_signal(self, symbol: str) -> Tuple[str, Dict[str, Any]]:
        ...
        try:
            ...
            if buy_votes >= 4 and buy_votes > sell_votes:
                decision = "BULLISH_FORCE"
            elif sell_votes >= 4 and sell_votes > buy_votes:
                decision = "BEARISH_FORCE"
            ...
            # Pretty-print voting summary
            _ema_b = '✅' if ema5 and ema9 and ema5 > ema9 else ('❌' if ema5 and ema9 and ema5 < ema9 else '➖')
            _vwap_b = '✅' if ltp and vwap and ltp > vwap * 1.001 else ('❌' if ltp and vwap and ltp < vwap * 0.999 else '➖')
            _st_b = '✅' if st == 1 else ('❌' if st == -1 else '➖')
            _rsi_b = '✅' if rsi > 50 else ('❌' if rsi < 50 else '➖')
            print(f"")
            print(f"{'='*70}")
            print(f"  📊 VOTING GATE — {symbol}")
            print(f"{'─'*70}")
            print(f"  ADX Gate     : {adx:.1f} {'✅ PASS (≥18 trending)' if adx >= 18 else '❌ FAIL (<18 no trend)'}")
            print(f"  1️⃣ EMA 5/9   : {_ema_b}  EMA5={ema5:.2f} vs EMA9={ema9:.2f}")
            print(f"  2️⃣ VWAP      : {_vwap_b}  LTP={ltp:.2f} vs VWAP={vwap:.2f}")
            print(f"  3️⃣ SuperTrend: {_st_b}  signal={int(st)}")
            print(f"  4️⃣ RSI       : {_rsi_b}  RSI={rsi:.1f}")
            print(f"{'─'*70}")
            print(f"  RESULT: BUY={buy_votes}/5 | SELL={sell_votes}/5 → {decision}")
            print(f"{'='*70}")
            print(f"")

            return decision, inds
        ...
        while True:
            ...
            try:
                ...
                if not self.om.active:
                    ...
                    if bias_str == "NEUTRAL":
                        # Avoid noisy open - do not enter before 09:25 IST (especially expiry)
                        now_ist = dt.datetime.now(self.IST) if hasattr(self, "IST") else dt.datetime.now()
                        # FIX #3: Full expiry time gates
                        # Determine expiry from days_to_expiry if present, else assume False.
                        try:
                            dte = int(getattr(self, "days_to_expiry", 7) or 7)
                        except Exception:
                            dte = 7
                        is_expiry_day = is_expiry_day_from_days(dte)

                        ok, reason = time_gate_ok(now_ist, is_expiry_day=is_expiry_day)
                        if not ok:
                            print(f"[TIME-GATE] {now_ist.strftime('%H:%M')} blocked: {reason} - skipping entries")
                            _time.sleep(self.poll_seconds)
                            continue
                    ...
                    with ThreadPoolExecutor(max_workers=1) as executor:
                        ...
                        # --- OI context from option-chain (if available) ---
                        try:
                            if proposed_signal is not None and hasattr(proposed_signal, 'meta') and isinstance(proposed_signal.meta, dict):
                                oi_meta = proposed_signal.meta.get('oi')
                                oi_chg_meta = proposed_signal.meta.get('oi_change')
                                inds_opt['oi'] = float(oi_meta) if oi_meta not in (None, "") else None
                                inds_opt['oi_change'] = float(oi_chg_meta) if oi_chg_meta not in (None, "") else None
                                print(f"[OI-CONTEXT] {option_symbol} | OI={inds_opt.get('oi')} | OI_Chg={inds_opt.get('oi_change')}")
                            else:
                                inds_opt['oi'] = None
                                inds_opt['oi_change'] = None
                                print(f"[OI-CONTEXT] {option_symbol} | No signal meta available")
                        except Exception as e:
                            inds_opt['oi'] = None
                            inds_opt['oi_change'] = None
                            print(f"[OI-CONTEXT] Failed to inject OI data for {option_symbol}: {e}")
                        ...
                        opt_st14 = int(inds_opt.get("st14_dir") or 0)
                        opt_st21 = int(inds_opt.get("st21_dir") or 0)
                        opt_close = float(inds_opt.get("close") or 0.0)
                        if (not efi_opt and not inds_opt.get("momentum_now")):
                            # Do not hard-skip if we still have tradable context from price/ST.
                            if opt_close > 0 and (opt_st14 != 0 or opt_st21 != 0):
                                print(
                                    f"[WARN] Option EFI/Mom not ready for {option_symbol}, "
                                    f"continuing with ST/price context (close={opt_close:.2f}, st14={opt_st14}, st21={opt_st21})"
                                )
                            else:
                                print(f"[SKIP] OHLC/Indicators NOT READY for OPTION {option_symbol} — skipping cycle")
                                _time.sleep(self.poll_seconds)
                                continue
                    ...
                    threshold = getattr(self, "_last_vote_threshold", 3)
                    voted_type = getattr(self, "_last_vote_option_type", desired_type)
                    if voted_type in ("CE", "PE") and voted_type != desired_type:
                        print(f"[VOTE] Direction mismatch (selected={desired_type}, voted={voted_type}) | SKIP")
                        _time.sleep(self.poll_seconds)
                        continue
                    if vote < threshold:
                        print(f"[VOTE] {resolved.analysis_symbol} | Bias={bias_str} | Opt-Vote={vote} < {threshold} | SKIP")
                        _time.sleep(self.poll_seconds)
                        continue
                    ...
                    signal.meta.update({
                        ...
                        "atr": inds_fut.get("atr", 0),
                        "atr_option": inds_opt.get("atr", 0), 
                        "pcr_index": pcr,
                    })
...
DEFAULT_CONFIGS_V2: Dict[str, UnderlyingConfigV2] = {
    ...
    "NATGASMINI": UnderlyingConfigV2(
        ...
        daily_loss_limit=3000.0,
        # Prefer strict closer-to-ATM as requested (0.50 delta)
        delta_target_abs=0.50,
        delta_min_abs=0.45,
        delta_max_abs=0.55,
        ...
        min_votes=3,
        order_lots=1,
        fallback_on_no_candidates=True,
        allow_no_greeks_fallback=True,
    ),
...
def run_v3_bot(symbol: str, simulate: bool = False):
    ...
    # Backward-compatible broker init (some versions don't accept paper_trade kwarg).
    try:
        broker = FyersBrokerAdapterV2(bot_v1.fyers_sdk_instance, paper_trade=simulate)
    except TypeError:
        broker = FyersBrokerAdapterV2(bot_v1.fyers_sdk_instance)
        if hasattr(broker, "paper_trade"):
            try:
                setattr(broker, "paper_trade", bool(simulate))
            except Exception:
                pass
